...
export const companyApi = {
  ...
  create: async (payload: { name: string; company_type: 'individual' | 'corporate'; short_name: string; description: string; is_active: boolean }) => apiClient<CompanyResponse>('/api/companies', { method: 'POST', body: payload }),
  update: async (id: number, payload: { name: string; company_type: 'individual' | 'corporate'; short_name: string; description: string; is_active: boolean }) => apiClient<CompanyResponse>(`/api/companies/${id}`, { method: 'PUT', body: payload }),
