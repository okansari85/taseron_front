...
const buildBusinessEntityForm = (payload: Record<string, any>) => {
 const form = new FormData()
 Object.entries(payload).forEach(([key, value]) => {
  if (value === undefined || value === null) return
  if (key === 'photos' && Array.isArray(value)) { value.forEach(v => { if (v instanceof File) form.append('photos[]', v) }); return }
  if (key === 'remove_photo_ids' && Array.isArray(value)) { value.forEach(v => form.append('remove_photo_ids[]', String(v))); return }
  if (key === 'brand_ids' && Array.isArray(value)) { value.forEach(v => form.append('brand_ids[]', String(v))); return }
...
export const locationApi = {
 ...
 multiBranchBuildings: () => apiClient<LocationApiItem[]>('/api/locations/multi-branch-buildings'),
 ...
 createBusinessEntity: async (locationId:number,payload:Record<string, any>) => { const hasFiles = Array.isArray(payload.photos) && payload.photos.some((f:any) => f instanceof File); const body = hasFiles ? buildBusinessEntityForm(payload) : payload; return unwrap<any>(await apiClient<any>(`/api/locations/${locationId}/business-entities`,{method:'POST',body})) },
 updateBusinessEntity: async (locationId:number,businessEntityId:number,payload:Record<string, any>) => { const hasFiles = Array.isArray(payload.photos) && payload.photos.some((f:any) => f instanceof File); const hasRemove = Array.isArray(payload.remove_photo_ids) && payload.remove_photo_ids.length>0; if (hasFiles || hasRemove) { const form = buildBusinessEntityForm(payload); form.append('_method','PUT'); return unwrap<LocationBusinessEntity>(await apiClient<any>(`/api/locations/${locationId}/business-entities/${businessEntityId}`,{method:'POST',body:form})) } return unwrap<LocationBusinessEntity>(await apiClient<any>(`/api/locations/${locationId}/business-entities/${businessEntityId}`,{method:'PUT',body:payload})) },
