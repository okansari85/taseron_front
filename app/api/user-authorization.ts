...
export type AuthorizationScope = {
  id: number
  scope_type: 'tenant' | 'organization' | 'location' | string
  scope_id: number
}
...
export const userAuthorizationApi = {
  ...
  assignRole: async (userId: number, role: string) => unwrap<AuthorizedUser>(await apiClient<ApiResponse<AuthorizedUser>>(`/api/users/${userId}/role`, {
    method: 'POST',
    body: { role },
  })),
  ...
  syncRolePermissions: async (role: string, permissions: string[]) => unwrap<AuthorizationRole>(await apiClient<ApiResponse<AuthorizationRole>>(`/api/roles/${encodeURIComponent(role)}/permissions`, {
    method: 'PUT',
    body: { permissions },
  })),
  ...
  syncScopes: async (userId: number, scopes: Array<{ scope_type: string; scope_id: number }>) => unwrap<AuthorizationScope[]>(await apiClient<ApiResponse<AuthorizationScope[]>>(`/api/users/${userId}/scopes`, {
    method: 'PUT',
    body: { scopes },
  })),
}
