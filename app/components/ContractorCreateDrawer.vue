<template>
  <Teleport to="body">
    <div v-if="modelValue" class="fixed inset-0 z-[9999]">
      <button aria-label="Yeni alt yüklenici panelini kapat" class="absolute inset-0 h-full w-full cursor-default bg-slate-950/35 backdrop-blur-[1px]" @click="close"></button>
      <aside class="absolute right-0 top-0 flex h-full w-full max-w-[560px] flex-col bg-white shadow-2xl dark:bg-gray-950">
        <div class="flex items-start justify-between border-b border-gray-200 px-6 py-5 dark:border-gray-800"><div class="flex items-start gap-3"><div class="flex h-11 w-11 items-center justify-center rounded-xl bg-brand-50 text-brand-500 dark:bg-brand-500/10"><Users :size="20" /></div><div><h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">Yeni Alt Yüklenici</h2><p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Tenant seviyesinde yeni bir alt yüklenici oluşturun.</p></div></div><button type="button" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100" @click="close"><X :size="22" /></button></div>
        <form class="flex min-h-0 flex-1 flex-col" @submit.prevent="submit"><div class="flex-1 space-y-6 overflow-y-auto px-6 py-6">
          <div><label class="mb-2 block text-sm font-semibold text-gray-800 dark:text-white/90">Firma Logosu</label><div class="flex items-center gap-4 rounded-xl border border-gray-200 p-4 dark:border-gray-800"><div class="flex h-24 w-24 shrink-0 items-center justify-center rounded-xl bg-gray-100 text-xl font-semibold text-gray-400 dark:bg-gray-800">{{ initials || 'AL' }}</div><div><p class="text-sm font-medium text-gray-800 dark:text-white/90">Logo</p><p class="mt-1 text-xs leading-5 text-gray-500">Mock avatar. Gerçek logo daha sonra eklenebilir.</p></div></div></div>
...
interface ContractorForm { name: string; shortName: string; type: ContractorType; status: ContractorStatus }
const props = withDefaults(defineProps<{ modelValue: boolean }>(), { modelValue: false })
const emit = defineEmits<{ 'update:modelValue': [value: boolean]; save: [payload: ContractorForm] }>()
const form = reactive<ContractorForm>({ name: '', shortName: '', type: 'Daimi', status: 'active' })
const initials = computed(() => form.shortName.trim().slice(0, 2).toLocaleUpperCase('tr-TR'))
const reset = () => Object.assign(form, { name: '', shortName: '', type: 'Daimi' as ContractorType, status: 'active' as ContractorStatus })
const close = () => emit('update:modelValue', false)
const submit = () => { emit('save', { ...form }); close(); reset() }
