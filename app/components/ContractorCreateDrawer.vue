    ...
    <div v-if="modelValue" class="fixed inset-0 z-[9999]">
      ...
      <aside class="absolute right-0 top-0 flex h-full w-full max-w-[560px] flex-col bg-white shadow-2xl dark:bg-gray-950">
        ...
        <form class="flex min-h-0 flex-1 flex-col" @submit.prevent="submit"><div class="flex-1 space-y-6 overflow-y-auto px-6 py-6">
          <div><label class="mb-2 block text-sm font-semibold text-gray-800 dark:text-white/90">Firma Logosu</label><label class="flex cursor-pointer items-center gap-4 rounded-xl border border-gray-200 p-4 dark:border-gray-800"><div class="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gray-100 text-xl font-semibold text-gray-400 dark:bg-gray-800"><img v-if="logoPreview && !logoPreviewError" :src="logoPreview" alt="Firma logosu önizleme" class="h-full w-full object-contain" @error="onLogoError" /><span v-else>{{ initials || 'AL' }}</span></div><div><p class="text-sm font-medium text-gray-800 dark:text-white/90">Logo</p><p class="mt-1 text-xs leading-5 text-gray-500">Logo yükleyin veya mevcut logoyu değiştirin.</p><p class="mt-1 text-[11px] text-gray-400">PNG, JPG, WEBP, SVG (maks. 5MB)</p><input type="file" class="hidden" accept="image/png,image/jpeg,image/webp,image/svg+xml" @change="onLogoChange" /></div></label></div>
          ...
          <div><label for="contractor-short-name" class="mb-2 block text-sm font-semibold text-gray-800 dark:text-white/90">Kısa Ad <span class="text-red-500">*</span></label><input id="contractor-short-name" v-model="form.shortName" required maxlength="100" placeholder="Örn. ISS" class="h-12 w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900" /></div>
