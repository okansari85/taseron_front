...
type GroupForm = { name: string; slug: string; code: string; description: string; display_order: number; is_active: boolean; parent_id: number | null; color: string; default_brand_id: number | null }
const props = withDefaults(defineProps<{ open: boolean; form: GroupForm; parentGroups?: Array<{ id: number; name: string }>; brandOptions?: Array<{ id: number; name: string; logo_url?: string | null }>; saving?: boolean; rootOrganizationName?: string | null; rootOrganizationId?: number | null }>(), { parentGroups: () => [], brandOptions: () => [], saving: false, rootOrganizationName: null, rootOrganizationId: null })
        ...
        <form class="flex min-h-0 flex-1 flex-col" @submit.prevent="submit">
          <div class="flex-1 space-y-5 overflow-y-auto px-6 py-6">
            ...
            <div>
              ...
              <select v-model.number="props.form.parent_id" class="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                <option v-if="props.rootOrganizationId !== null && props.rootOrganizationId !== undefined" :value="props.rootOrganizationId">{{ props.rootOrganizationName || 'En üst seviye' }}</option>
            ...
            <div><label class="mb-2 block text-sm font-semibold text-gray-800 dark:text-white/90">Görünüm</label><div class="rounded-xl border border-gray-200 p-4 dark:border-gray-800"><p class="text-sm font-medium text-gray-700 dark:text-gray-300">İkon Rengi</p><div class="mt-3 flex items-center gap-3"><button v-for="color in colorOptions" :key="color" type="button" class="flex h-7 w-7 items-center justify-center rounded-full transition" :class="normalizeColor(props.form.color) === normalizeColor(color) ? 'ring-2 ring-brand-500 ring-offset-2 dark:ring-offset-gray-950' : ''" :style="{ backgroundColor: color }" :aria-label="`İkon rengi ${color}`" @click="props.form.color = color"><span v-if="normalizeColor(props.form.color) === normalizeColor(color)" class="h-1.5 w-1.5 rounded-full bg-white" /></button></div><p class="mt-2 text-xs text-gray-400">Listelerde ve hiyerarşide kullanılacak vurgu rengi.</p></div></div>
            <div><label class="mb-2 block text-sm font-semibold text-gray-800 dark:text-white/90">Varsayılan Marka</label><select v-model.number="props.form.default_brand_id" class="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"><option :value="null">Seçilmedi</option><option v-for="brand in props.brandOptions" :key="brand.id" :value="brand.id">{{ brand.name }}</option></select><p class="mt-1 text-xs text-gray-400">Bu grubun menüsünde ve üst logoda gösterilecek marka.</p></div>
