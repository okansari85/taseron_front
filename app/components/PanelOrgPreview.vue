      ...
      <div class="absolute left-1/2 top-0 origin-top transition-transform duration-150" :class="isDragging ? 'duration-0' : ''" :style="chartTransform">
        <div ref="chartCanvas" class="relative">
          <svg v-if="connectorLines.length" class="tenant-connectors" :width="connectorSize.width" :height="connectorSize.height" :viewBox="`0 0 ${connectorSize.width} ${connectorSize.height}`" aria-hidden="true">
            <path v-for="(line, index) in connectorLines" :key="index" :d="line" fill="none" stroke="#8B7CFF" stroke-width="1" stroke-dasharray="4 4" stroke-linecap="butt" vector-effect="non-scaling-stroke" />
          </svg>

          <OrganizationChart :data="orgData" :default-expand-all="true" class="tenant-org-chart" @select="handleNodeSelect">
            <template #node-title="{ node }">
              <div :class="['tenant-node-content', `tenant-node-content--${node.meta?.tone ?? 'purple'}`]">
                <span class="tenant-node-icon" aria-hidden="true">
                  <Network v-if="node.meta?.icon === 'organization'" :size="20" :stroke-width="2" />
                  <Building2 v-else-if="node.meta?.icon === 'company'" :size="20" :stroke-width="2" />
                  <MapPin v-else-if="node.meta?.icon === 'location'" :size="20" :stroke-width="2" />
                  <GitBranch v-else :size="20" :stroke-width="2" />
                </span>
...
import { Building2, GitBranch, MapPin, Network } from '@lucide/vue'
...
function visibleNode(node: HTMLElement) {
  const style = window.getComputedStyle(node)
  return node.isConnected && style.display !== 'none' && style.visibility !== 'hidden' && node.getClientRects().length > 0
}
...
const scheduleConnectorRender = () => {
  ...
  connectorRenderTimer = setTimeout(() => renderConnectors(), 40)
}
...
.tenant-org-chart .tenant-node-title { width: 156px; max-width: 156px; min-height: 16px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-align: center; font-size: 13px; font-weight: 700; line-height: 16px; color: #1f2937; }
