<template>
  <aside class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div ref="chartWrap" class="relative mt-5 h-[260px] overflow-hidden rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-900" :class="isDragging ? 'cursor-grabbing' : 'cursor-grab'" @wheel.prevent="zoomWithWheel" @mousedown="startPan" @mousemove="movePan" @mouseup="endPan" @mouseleave="endPan">
      <div class="absolute left-1/2 top-0 origin-top transition-transform duration-150" :class="isDragging ? 'duration-0' : ''" :style="chartTransform">
        <div ref="chartCanvas" class="relative">
          <svg v-if="connectorLines.length" class="tenant-connectors" :width="connectorSize.width" :height="connectorSize.height" :viewBox="`0 0 ${connectorSize.width} ${connectorSize.height}`" aria-hidden="true">
            <path v-for="(line, index) in connectorLines" :key="index" :d="line" fill="none" stroke="#8B7CFF" stroke-width="1" stroke-dasharray="4 4" stroke-linecap="round" vector-effect="non-scaling-stroke" />
          </svg>

          <OrganizationChart :data="orgData" :default-expand-all="true" class="tenant-org-chart" @click-node="handleNodeClick">
            <template #node-title="{ node }">
              <div :class="['tenant-node-content', `tenant-node-content--${node.meta?.tone ?? 'purple'}`]">
                <span class="tenant-node-icon" aria-hidden="true">
                  <svg v-if="node.meta?.icon === 'organization'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><rect x="5" y="3" width="14" height="6" rx="1.5" /><path d="M12 9v4M12 13H7v4M12 13h5v4M4.5 20h5M14.5 20h5" /><rect x="3" y="17" width="6" height="3" rx="1" /><rect x="15" y="17" width="6" height="3" rx="1" /></svg>
                  <svg v-else-if="node.meta?.icon === 'company'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><path d="M4 21V5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v16" /><path d="M3 21h18M8 7h2M14 7h2M8 11h2M14 11h2M8 15h2M14 15h2" /></svg>
                  <svg v-else-if="node.meta?.icon === 'location'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><path d="M20 10.2c0 5.4-8 11-8 11s-8-5.6-8-11a8 8 0 1 1 16 0Z" /><circle cx="12" cy="10.2" r="2.4" /></svg>
                  <svg v-else viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><path d="M12 3l4 5-4 5-4-5 4-5Z" /><path d="M12 13v8M5 20h14" /></svg>
                </span>
      ...
      <div class="absolute right-3 top-3 rounded-lg border border-gray-200 bg-white/90 px-2 py-1 text-[11px] text-gray-500 shadow-theme-xs dark:border-gray-700 dark:bg-gray-800/90 dark:text-gray-400">Tekerlek ile yakınlaştır</div>
    </div>
    ...
    <div class="space-y-3">
      <div v-for="row in legend" :key="row.title" class="flex items-start gap-3">
        ...
        <div><div :class="['text-theme-xs font-semibold', row.text]">{{ row.title }}</div><div class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ row.desc }}</div></div>
      </div>
    </div>

    <div class="mt-4 rounded-lg border border-brand-100 bg-brand-50 px-4 py-3 dark:border-brand-500/20 dark:bg-brand-500/10"><div class="text-sm font-semibold text-brand-600 dark:text-brand-400">Bilgi</div><div class="mt-1 text-theme-xs text-brand-700 dark:text-brand-400">Seçtiğiniz kurumsal yapıya göre ilk yapılandırma burada gösterilir. Şahıs şirketinde merkez lokasyon otomatik oluşturulur.</div></div>
  </aside>
</template>
...
const renderConnectors = async () => {
  ...
  requestAnimationFrame(() => {
    ...
    const rects = nodes.map((node) => { const rect = node.getBoundingClientRect(); return { top: rect.top - canvasRect.top, bottom: rect.bottom - canvasRect.top, centerX: rect.left - canvasRect.left + rect.width / 2 } })
    const root = rects[0]
    ...
    connectorSize.value = { width: Math.max(canvas.offsetWidth, maxX + 4), height: Math.max(canvas.offsetHeight, childTop + 4) }
  })
}
...
<style>
.tenant-org-chart, .tenant-org-chart .org-table { display: flex !important; justify-content: center !important; margin: 0 auto !important; }
...
.tenant-org-chart .org-child-level::before, .tenant-org-chart .org-child-level::after, .tenant-org-chart .org-extend::after, .tenant-org-chart .org-extend-arrow::before { display: none !important; content: none !important; }
...
.tenant-connectors { position: absolute; inset: 0; pointer-events: none !important; overflow: visible; z-index: 0; }
