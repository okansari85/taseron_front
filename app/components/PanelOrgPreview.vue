...
type PreviewMeta = {
  type: string
  textClass: string
  icon: 'organization' | 'company' | 'location' | 'group'
...
const orgData = computed<PreviewNode>(() => {
  ...
  if (onboardingType.value === 'company') {
    children.push({
      ...
      member: [],
      meta: { type: 'Şirket', textClass: 'text-success-700 dark:text-success-400', icon: 'company', tone: 'green' },
    })
    ...
    if (isSahisSirketi.value) {
      children.push({
        ...
        member: [],
        meta: { type: 'Lokasyon', textClass: 'text-warning-700 dark:text-warning-400', icon: 'location', tone: 'orange', automatic: true },
      })
    }
  }
  ...
  if (onboardingType.value === 'brand') {
    children.push({
      ...
      title: rootLabel.value,
      member: [],
      meta: { type: 'Marka', textClass: 'text-blue-light-600 dark:text-blue-light-400', icon: 'group', tone: 'blue' },
    })
  }

  if (onboardingType.value === 'group') {
    return { id: 'group', title: rootLabel.value, member: [], meta: { type: 'Grup', textClass: 'text-brand-500 dark:text-brand-400', icon: 'group', tone: 'purple' } }
  }

  if (onboardingType.value === 'holding') {
    return { id: 'holding', title: rootLabel.value, member: [], meta: { type: 'Holding', textClass: 'text-brand-500 dark:text-brand-400', icon: 'organization', tone: 'purple' } }
  }
...
const renderConnectors = async () => {
  ...
  requestAnimationFrame(() => {
    ...
    const nodes = Array.from(canvas.querySelectorAll('.tenant-org-chart .org-container'))
      .filter((node) => visibleNode(node as HTMLElement)) as HTMLElement[]
...
.tenant-org-chart .org-extend-arrow::before {
