<template>
  <header
    class="sticky top-0 z-[999] w-full border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900"
  >
    <div
      class="flex w-full items-center justify-between px-4 py-3 lg:px-6 lg:py-3.5"
    >
      <div class="flex min-w-0 items-center gap-3">
        <button
          class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-gray-200 text-gray-600 dark:border-gray-800 dark:text-gray-300 lg:h-11 lg:w-11"
          @click="handleSidebar"
        >
          <Menu :size="20" />
        </button>
        <div class="hidden min-w-0 sm:block">
          ...
          >
            <template
              ...
              ><button
                type="button"
                class="truncate transition hover:text-brand-500"
                :class="
          ...
          <p
            class="truncate text-[10px] font-medium text-gray-400 dark:text-gray-500"
          >
            Taseron Management
          </p>
        </div>
      ...
      <div class="flex items-center gap-2">
        <div
          v-if="isTenantWorkspace && currentTenantOption"
        ...
        >
          ...
          >
            <span class="flex min-w-0 items-center gap-2.5"
              ><span
                class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-brand-50 text-brand-500 dark:bg-brand-500/10 dark:text-brand-400"
                ><component :is="currentTenantOption.icon" :size="15" /></span
              ><span class="min-w-0"
                ><span
                  class="block text-[9px] font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500"
                  >Tenant</span
                ><span
                  class="block truncate text-xs font-semibold text-gray-800 dark:text-white/90"
                  >{{ currentTenantOption.name }}</span
                ></span
              ></span
          ...
          >
            ...
            <div class="max-h-72 overflow-y-auto p-1.5">
              <button
                ...
                :class="
                  Number(item.id) === Number(tenantId)
              ...
              >
                <span
                  class="flex h-7 w-7 items-center justify-center rounded-md bg-brand-50 text-brand-500 dark:bg-brand-500/10 dark:text-brand-400"
                  ><Building2 :size="13" /></span
                ><span class="min-w-0 flex-1"
                  ><span
                    class="block truncate text-xs font-semibold text-gray-700 dark:text-gray-200"
                    >{{ item.name }}</span
                ...
                ><Check
                  v-if="Number(item.id) === Number(tenantId)"
                  :size="14"
        ...
        <div class="relative hidden sm:block">
          ...
          >
            <div
              class="flex h-9 w-9 items-center justify-center rounded-full bg-brand-50 text-xs font-semibold text-brand-500 dark:bg-brand-500/15 dark:text-brand-400"
            >
              {{ initials }}
            </div>
            <div>
...
const displayBreadcrumbs = computed(() =>
  route.path.includes("/locations/") && tenantLocationContext.value
    ? [
        ...(currentTenantOption.value
          ? [
              {
                kind: "tenant" as const,
                id: currentTenantOption.value.id,
                name: currentTenantOption.value.name,
              },
            ]
          : []),
        { kind: "locations" as const, id: "locations", name: "Lokasyonlar" },
        {
          ...
          id: tenantLocationContext.value.id,
          name: tenantLocationContext.value.name,
        },
      ]
    : breadcrumbs.value,
);
watch(
  isTenantWorkspace,
  async (enabled) => {
    if (enabled && tenantStore.tenants.length === 0)
...
const selectTenant = async (id: number | string) => {
  ...
  const selected = tenantStore.tenants.find(
    (item) => Number(item.id) === Number(id),
  );
  if (selected) tenantStore.currentTenant = selected;
  if (Number(id) === Number(tenantId.value)) return;
  const suffix = route.path.split(`/tenants/${tenantId.value}`)[1] || "";
  await router.push(`/tenants/${id}${suffix}`);
};
...
const handleBreadcrumb = (item: (typeof displayBreadcrumbs.value)[number]) => {
  if (item.kind === "locations")
    router.push(`/tenants/${tenantId.value}/locations`);
  else goToBreadcrumb(item.kind);
};
