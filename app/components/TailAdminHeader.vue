  ...
  <header class="sticky top-0 z-[999] w-full border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
    <div class="flex w-full items-center justify-between px-4 py-3 lg:px-6 lg:py-3.5">
      ...
      <div class="flex items-center gap-2">
        <!-- Önceki kapsam: breadcrumb'dan bağımsız, ayrı ve seçilebilir parent scope -->
        <div v-if="isTenantWorkspace && previousScope" class="relative hidden lg:block">
          <button
            type="button"
            class="flex min-w-[190px] max-w-[270px] items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white px-3 py-2 text-left shadow-sm transition hover:border-brand-200 dark:border-gray-800 dark:bg-gray-900 dark:hover:border-brand-500/40"
            :title="`Üst kapsam: ${previousScope.name}`"
            @click="previousScopeOpen = !previousScopeOpen"
          >
            <span class="flex min-w-0 items-center gap-2.5">
              <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-gray-50 text-gray-500 dark:bg-white/5 dark:text-gray-400">
                <component :is="scopeIcon(previousScope.kind)" :size="15" />
              </span>
              <span class="min-w-0">
                <span class="block text-[9px] font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">{{ scopeKindLabel(previousScope.kind) }}</span>
                <span class="block truncate text-xs font-semibold text-gray-800 dark:text-white/90">{{ previousScope.name }}</span>
              </span>
            </span>
            <ChevronDown :size="15" class="shrink-0 text-gray-400 transition-transform" :class="previousScopeOpen ? 'rotate-180' : ''" />
          </button>

          <div v-if="previousScopeOpen" class="absolute right-0 top-full z-[1000] mt-2 w-80 overflow-hidden rounded-xl border border-gray-200 bg-white p-1.5 shadow-xl dark:border-gray-800 dark:bg-gray-900">
            <div class="border-b border-gray-100 px-3 py-2.5 dark:border-gray-800">
              <p class="text-[10px] font-semibold uppercase tracking-wide text-gray-400">{{ scopeKindLabel(previousScope.kind) }} kapsamı</p>
              <p class="mt-0.5 text-[11px] text-gray-500 dark:text-gray-400">Bir üst kapsamı seçerek hızlıca geri çıkın.</p>
            </div>
            <div class="max-h-72 overflow-y-auto px-1.5 py-1.5">
              <button
                v-for="option in previousScopeOptions"
                :key="option.id"
                type="button"
                ...
                :class="option.id === previousScope.id ? 'bg-brand-50/70 dark:bg-brand-500/10' : ''"
                @click="selectPreviousScope(option)"
              >
                ...
                <Check v-if="option.id === previousScope.id" :size="14" class="shrink-0 text-brand-500" />
              </button>
            </div>
          </div>
        </div>
...
const previousScopeOpen = ref(false)
...
const previousScope = computed(() => {
  if (breadcrumbs.value.length < 2) return null
  return breadcrumbs.value[breadcrumbs.value.length - 2]
})
const previousScopeOptions = computed<ScopeOption[]>(() => {
  if (!previousScope.value) return []
  return optionsForKind(previousScope.value.kind) as ScopeOption[]
})
...
const selectPreviousScope = (option: ScopeOption) => {
  if (!previousScope.value) return
  setScope(previousScope.value.kind, option)
  previousScopeOpen.value = false
  selectedKind.value = previousScope.value.kind
}
