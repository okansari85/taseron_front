  ...
  <header class="sticky top-0 z-[999] w-full border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
    <div class="flex w-full items-center justify-between px-4 py-3 lg:px-6 lg:py-3.5">
        ...
        <div class="hidden min-w-0 sm:block">
          <div class="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500">
            <template v-for="(item, index) in breadcrumbs" :key="`${item.kind}-${item.id}`">
              <button type="button" class="truncate transition hover:text-brand-500" :class="index === breadcrumbs.length - 1 ? 'font-medium text-gray-600 dark:text-gray-300' : ''" @click="goToBreadcrumb(item.kind)">{{ item.name }}</button>
      ...
      <div class="flex items-center gap-2">
        ...
        <!-- Lokasyon içindeyken operasyonel context -->
        <div v-if="isTenantWorkspace && tenantLocationContext" class="hidden items-center gap-2 lg:flex">
          <div class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 dark:border-gray-800 dark:bg-gray-900">
            <Factory :size="15" class="text-brand-500" />
            <div class="min-w-0"><span class="block text-[9px] font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">Tesis</span><span class="block max-w-[180px] truncate text-xs font-semibold text-gray-800 dark:text-white/90">{{ tenantLocationContext.name }}</span></div>
          </div>
          <div v-if="operationalAreaContext" class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 dark:border-gray-800 dark:bg-gray-900">
            <Layers3 :size="15" class="text-brand-500" />
            <div class="min-w-0"><span class="block text-[9px] font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">Operasyonel Alan</span><span class="block max-w-[180px] truncate text-xs font-semibold text-gray-800 dark:text-white/90">{{ operationalAreaContext.name }}</span></div>
          </div>
        </div>
...
import { Bell, Check, ChevronDown, Factory, Layers3, Menu, Moon, Sun } from '@lucide/vue'
...
const { currentTenantOption, tenantLocationContext, operationalAreaContext, breadcrumbs, goToBreadcrumb } = useOrganizationScope()
