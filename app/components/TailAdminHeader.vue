  ...
  <header class="sticky top-0 z-[999] w-full border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
    <div class="flex w-full items-center justify-between px-4 py-3 lg:px-6 lg:py-3.5">
        ...
        <div class="hidden min-w-0 sm:block">
          <div class="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500">
            <button
              ...
              :class="index === breadcrumbs.length - 1 ? 'font-medium text-gray-600 dark:text-gray-300' : ''"
              @click="goToBreadcrumb(item.type, item.id)"
            >
              {{ item.name }}
              <span v-if="index < breadcrumbs.length - 1" class="mx-1.5 text-gray-300 dark:text-gray-700">/</span>
            </button>
          </div>
      ...
      <div class="flex items-center gap-2">
        <div v-if="isTenantWorkspace" class="relative hidden lg:block">
          <button
          ...
          <div v-if="scopeSelectorOpen" class="absolute right-0 top-full z-[1000] mt-2 w-72 overflow-hidden rounded-xl border border-gray-200 bg-white p-1.5 shadow-xl dark:border-gray-800 dark:bg-gray-900">
            ...
            <button
              ...
              @click="selectScopeOption(option)"
            >
...
import { Bell, Building2, ChevronDown, Factory, Layers3, Menu, Moon, Sun, UsersRound } from '@lucide/vue'

type ScopeLevel = 'tenant' | 'group' | 'company' | 'location'
...
const companyOptions: Record<string, ScopeOption[]> = {
  ...
  automotive: [
    ...
    { id: 'toss', name: 'Tofaş Türk Otomobil Fabrikası A.Ş.', description: 'Şirket kapsamına geç', icon: Building2 },
  ],
  energy: [
...
const currentScopeLabel = computed(() => {
  if (scopeLevel.value === 'tenant') return 'Workspace'
  if (scopeLevel.value === 'group') return 'Grup'
  if (scopeLevel.value === 'company') return 'Şirket'
  return 'Lokasyon'
})
...
const currentScopeIcon = computed(() => {
  if (scopeLevel.value === 'tenant') return Building2
  if (scopeLevel.value === 'group') return Layers3
  if (scopeLevel.value === 'company') return Building2
  return Factory
})

const nextScopeLabel = computed(() => {
  if (scopeLevel.value === 'tenant') return 'Grup'
  if (scopeLevel.value === 'group') return 'Şirket'
  if (scopeLevel.value === 'company') return 'Lokasyon'
  return 'Lokasyon'
})
...
const currentScopeOptions = computed<ScopeOption[]>(() => {
  ...
  if (scopeLevel.value === 'company') return locationOptions[selectedCompany.value?.id as string] || []
  return []
})
...
const breadcrumbs = computed<Breadcrumb[]>(() => {
  ...
  if (selectedCompany.value && ['company', 'location'].includes(scopeLevel.value)) items.push({ type: 'company', id: selectedCompany.value.id, name: selectedCompany.value.name })
...
const selectScopeOption = async (option: ScopeOption) => {
...
const goToBreadcrumb = (type: ScopeLevel, id: string | number) => {
  if (type === 'tenant') {
