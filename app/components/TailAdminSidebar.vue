<template>
  <div>
    ...
    >
      <div
        :class="['flex py-8', isExpanded ? 'justify-start' : 'justify-center']"
      >
      ...
      <nav class="no-scrollbar flex flex-1 flex-col overflow-y-auto pb-6">
        ...
        <div
          ...
          class="mt-6 flex flex-col border-t border-gray-100 pt-5 dark:border-gray-800"
        >
          <p
            v-if="isExpanded"
            ...
            <template v-for="item in workspaceItems" :key="item.path"
              ...
              <div v-else>
                <button
                  v-if="isExpanded"
                  type="button"
                  class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300"
          ...
          <div
            v-if="isExpanded"
            class="mt-3 mb-5 rounded-xl border border-gray-200 bg-gray-50/70 px-3 py-3 dark:border-gray-800 dark:bg-white/[0.03]"
          >
            <div class="flex items-start justify-between gap-2">
              <div class="min-w-0">
                <div
                  class="flex items-center gap-1.5 text-[10px] font-medium uppercase tracking-wide text-gray-400"
                >
                  <Building2 :size="10" /><span>Aktif Workspace</span>
                </div>
                <p
                  class="mt-1 truncate text-sm font-semibold text-gray-800 dark:text-white/90"
                >
                  {{ tenantName }}
                </p>
              </div>
              <span
                class="inline-flex shrink-0 items-center gap-1 rounded-full bg-success-50 px-2 py-1 text-[9px] font-semibold text-success-600"
                ><span class="h-1.5 w-1.5 rounded-full bg-success-500"></span
                >Aktif</span
              >
            </div>
            <div
              class="mt-3 border-t border-gray-200 dark:border-gray-800"
            ></div>
            <NuxtLink
              :to="`/tenants/${tenantId}/settings/workspace`"
              class="mt-3 flex items-center gap-2 rounded-lg px-2 py-1.5 text-[10px] font-medium text-brand-500"
              @click="closeMobile"
              ><Settings :size="12" /><span>Workspace Ayarları</span></NuxtLink
            >
          </div>
        </div>
        <div class="mt-auto border-t border-gray-100 pt-4 dark:border-gray-800">
          ...
          >
            <div
              class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-50 text-xs font-semibold text-brand-500"
            >
              AY
            </div>
            <div v-if="isExpanded" class="min-w-0">
              ...
              <p class="truncate text-xs text-gray-500">Sistem Yöneticisi</p>
            </div>
          </div>
        </div>
      </nav>
...
const tenantName = computed(
  () => currentTenant.value?.name ?? "Tenant yükleniyor...",
);
const rootOrganization = computed(
  () => currentTenant.value?.root_organization ?? null,
);
const workspaceItems = computed<WorkspaceMenuItem[]>(() => {
  const id = tenantId.value;
  if (!id || !rootOrganization.value) return [];
  const basePath = `/tenants/${id}`,
    organizationPath = `${basePath}/organization`;
  const menuItems: WorkspaceMenuItem[] = [
    { title: "Genel Durum", path: basePath, icon: Home },
  ];
  ...
  menuItems.push(
    ...
    { title: "Alt Yükleniciler", path: `${basePath}/contractors`, icon: Users },
