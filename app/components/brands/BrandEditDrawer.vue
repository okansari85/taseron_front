...
import { Check, FileImage, LockKeyhole, Save, X } from 'lucide-vue-next'
import type { Company } from '~/types/company'
import BrandCompanyMultiSelect from './BrandCompanyMultiSelect.vue'
...
const selectedCompanies = computed(() => props.companies.filter(company => props.form.companyIds.includes(company.id)))
const group = computed(() => selectedCompanies.value[0]?.group ?? '')
const close = () => emit('update:open', false)
const submit = () => emit('save', { ...props.form, companyIds: [...props.form.companyIds] })
        ...
        <form class="flex min-h-0 flex-1 flex-col" @submit.prevent="submit">
          <div class="flex-1 overflow-y-auto px-6 py-6">
            ...
            <div class="grid grid-cols-1 gap-x-4 gap-y-5 md:grid-cols-2">
              ...
              <label class="block"><span class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Şirket <span class="text-error-500">*</span></span><BrandCompanyMultiSelect v-model="form.companyIds" :companies="companies" /><span class="mt-1.5 block text-xs text-gray-500">Markanın bağlı olduğu şirketleri seçiniz.</span></label>
          ...
          <div class="flex items-center justify-end gap-3 border-t border-gray-200 px-6 py-4 dark:border-gray-800"><button type="button" class="inline-flex h-10 items-center gap-2 rounded-lg border border-gray-200 bg-white px-5 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300" @click="close"><X :size="16" />İptal</button><button type="submit" :disabled="saving || !form.companyIds.length" class="inline-flex h-10 items-center gap-2 rounded-lg bg-brand-500 px-6 text-sm font-semibold text-white shadow-theme-xs transition hover:bg-brand-600 disabled:opacity-60"><Save :size="16" />{{ saving ? 'Kaydediliyor...' : 'Kaydet' }}</button></div>
        </form>
      </aside>
    </div>
  </Transition>
