    ...
    <div class="flex min-h-[520px] flex-col md:flex-row">
      ...
      <div class="min-w-0 flex-1 p-6">
        ...
        <template v-else>
          <div class="mb-5 flex items-center justify-between gap-4">
            <div>
              <h2 class="text-sm font-semibold text-gray-900">Lokasyon Bazlı Evrak Durumu</h2>
              <p class="mt-1 text-xs text-gray-500">Her lokasyondaki personel, araç, ekipman ve kimyasalların genel evrak uygunluğu.</p>
            </div>
            <div class="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-500">
              <MapPin :size="14" /> {{ locationSummaries.length }} lokasyon
            </div>
          </div>

          <div class="space-y-3">
            <div v-for="location in locationSummaries" :key="location.name" class="rounded-xl border border-gray-100 bg-gray-50/40 p-4">
              <div class="mb-4 flex items-center gap-3">
                <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-50 text-brand-600"><MapPin :size="16" /></div>
                <div>
                  <p class="text-sm font-semibold text-gray-800">{{ location.name }}</p>
                  <p class="mt-0.5 text-[11px] text-gray-400">Genel evrak uygunluk durumu</p>
                </div>
              </div>

              <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div v-for="item in [{label:'Personel',value:location.personnel},{label:'Araç',value:location.vehicles},{label:'Ekipman',value:location.equipment},{label:'Kimyasal',value:location.chemicals}]" :key="item.label">
                  <div class="mb-1.5 flex items-center justify-between gap-2 text-[11px]">
                    <span class="font-medium text-gray-600">{{ item.label }}</span>
                    <span class="font-semibold" :class="item.value===100?'text-success-600':item.value>=90?'text-brand-600':'text-warning-600'">{{ item.value }}%</span>
                  </div>
                  <div class="h-2 overflow-hidden rounded-full bg-gray-100">
                    <div class="h-full rounded-full transition-all" :class="item.value===100?'bg-success-500':item.value>=90?'bg-brand-500':'bg-warning-500'" :style="{ width: `${item.value}%` }"></div>
                  </div>
                </div>
              </div>
            </div>
