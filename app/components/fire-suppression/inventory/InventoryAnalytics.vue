...
<template>
  <section class="mt-5 grid gap-3 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,1.15fr)_minmax(300px,.9fr)]">
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900"><h3 class="text-sm font-bold text-[#14284f] dark:text-white">Tesisat Bilgileri (Rapor'dan)</h3><div class="mt-3 divide-y divide-slate-100 dark:divide-gray-800"><div v-for="info in facilityInfo" :key="info.label" class="flex items-center justify-between gap-4 py-2 text-xs"><span class="text-slate-400">{{info.label}}</span><strong class="text-right text-[#14284f] dark:text-white">{{info.value}}</strong></div></div></div>
  </section>
</template>
