...
export type MatchRow = { equipmentIndex: number; code: string | null; categoryLabel: string | null; locationNote: string | null; properties?: Record<string, string> | null; bucket: MatchBucket }
const props = defineProps<{ rows: MatchRow[]; resolutionLabel?: (equipmentIndex: number) => string | null; newItemApproved?: (equipmentIndex: number) => boolean; section?: 'systems' | 'components'; reportCategory?: string | null }>()
...
// Kategorisi çözülemeyen (örn. YSC/tüp raporu - sistem adı hiçbir zaman
// gerçek bir kategoriye oturmadığı için hep "Diğer" gelir) bileşenler için
// "Diğer" rozetini zorlamak yerine, table_shape'in zaten çıkardığı GERÇEK
// özellikleri (Cihaz Tipi, Bulunduğu Yer gibi) göster - tüp için kategori
// anlamsızdır ama cihaz tipi/konum olmazsa olmazdır. Gerçek bir kategorisi
// olan bileşenler (dolap/pompa/hidrant) eski category+konum gösterimini
// KORUR, davranış onlarda değişmez.
const isUnresolvedCategory = (row: MatchRow) => !row.categoryLabel || row.categoryLabel === FIRE_SUPPRESSION_CATEGORY_LABELS.diger
// YSC (tüp) raporlarında bileşenin kodu bariz bir çıplak sayıdır (Tüp No) -
// hangi kimlik olduğu belirtilmeden sadece "5" gibi göstermek kafa
// karıştırıyordu. Diğer rapor tiplerinde kod zaten kendi anlamıyla (örn.
// "YD14") gösterilmeye devam eder.
const rowPrimaryLabel = (row: MatchRow): string => {
  if (!row.code) return 'Kod belirtilmemiş'
  return props.reportCategory === 'ysc' ? `Tüp No: ${row.code}` : row.code
}
const rowSubtitle = (row: MatchRow): string => {
  if (isUnresolvedCategory(row) && row.properties && Object.keys(row.properties).length) {
    return Object.entries(row.properties).map(([key, value]) => `${key}: ${value}`).join(' · ')
  }
  return `${row.categoryLabel || '—'} · ${row.locationNote || 'Konum belirtilmemiş'}`
}
</script>
