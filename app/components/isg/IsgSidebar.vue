          ...
          <div class="flex flex-col gap-1.5">
            <template v-for="item in section.items" :key="item.title">
              <button
                ...
                type="button"
                :class="[
                  'menu-item group w-full rounded-lg transition-all',
                  desktop ? (hasActiveChild(item) ? 'bg-[#3a1012] text-white' : 'text-white/75 hover:bg-white/[0.06] hover:text-white') : 'menu-item-inactive',
                  isExpanded ? 'justify-start' : 'justify-center'
                ]"
                @click="isExpanded ? toggleGroup(item.title) : navigateTo(item.children[0]?.path || item.path)"
              >
              ...
              <div v-if="'children' in item && item.children && isExpanded && expandedGroups.has(item.title)" class="ml-3 flex flex-col gap-1 border-l border-white/10 pl-3">
                <NuxtLink
                  ...
                  :class="[
                    ...
                    isActive(child.path)
                      ? 'bg-[#ef1015] text-white shadow-[0_6px_18px_rgba(239,16,21,0.22)]'
                      : (desktop ? 'text-white/65 hover:bg-white/[0.06] hover:text-white' : 'text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-white/5')
                  ]"
                  @click="closeMobile"
                >
                  {{ child.title }}
              ...
              <NuxtLink
                ...
                :to="item.path"
                :class="['menu-item group rounded-lg', isActive(item.path) ? (desktop ? 'text-white shadow-[0_8px_24px_rgba(15,23,42,0.22)]' : 'menu-item-active') : (desktop ? 'text-white/75 hover:bg-white/[0.06] hover:text-white' : 'menu-item-inactive'), isExpanded ? 'justify-start' : 'justify-center']"
                :style="isActive(item.path) && desktop ? { backgroundColor: primaryColor } : undefined"
                @click="closeMobile"
              >
                <component :is="item.icon" :size="18" />
  ...
  { title: 'Yangın Yönetimi', items: [
    {
      ...
      path: '/isg-portal/desktop/ysc',
      icon: Flame,
      children: [
        { title: 'Yangın Söndürücüler (YSC)', path: '/isg-portal/desktop/ysc' },
