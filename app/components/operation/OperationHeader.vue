    ...
    <div class="flex w-full items-center justify-between px-4 py-3 lg:px-6 lg:py-3.5">
      <div class="flex min-w-0 items-center gap-3">
        ...
        <div class="hidden min-w-0 sm:block">
          <p class="truncate text-sm font-semibold text-gray-800 dark:text-white/90">Operasyon Portalı</p>
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="relative hidden sm:block">
          <button class="flex items-center gap-3 border-l border-gray-200 pl-3 text-left dark:border-gray-800" @click="profileOpen = !profileOpen">
            <div class="flex h-9 w-9 items-center justify-center rounded-full bg-brand-50 text-xs font-semibold text-brand-500">{{ initials }}</div>
            <div>
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ auth.user.value?.name || 'Kullanıcı' }}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400">Operasyon</p>
            </div>
          </button>
          <div
            v-if="profileOpen"
...
import { useOperationSidebar } from '~/composables/useOperationSidebar'

const router = useRouter()
const auth = useAuth()
const { toggle, toggleMobile } = useOperationSidebar()
...
const handleLogout = async () => {
  ...
  await auth.logout()
  await router.push('/operation-portal/login')
}
</script>
