  ...
  <div>
    ...
    >
      <div :class="['flex py-8', isExpanded ? 'justify-start' : 'justify-center']">
        <NuxtLink to="/operation-portal/work-requests" class="flex items-center gap-3">
          <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-500 text-sm font-bold text-white">O</span>
          <span v-if="isExpanded" class="text-lg font-semibold text-gray-800 dark:text-white/90">Operasyon Portalı</span>
        </NuxtLink>
      </div>
        ...
        <div class="mt-auto border-t border-gray-100 pt-4 dark:border-gray-800">
          <div :class="['flex items-center gap-3 rounded-lg px-2 py-3', isExpanded ? 'justify-start' : 'justify-center']">
            ...
            <div v-if="isExpanded" class="min-w-0">
              ...
              <p class="truncate text-xs text-gray-500 dark:text-gray-400">Operasyon</p>
            </div>
          </div>
        </div>
      </nav>
...
const auth = useAuth()
const { isExpanded, isMobileOpen, closeMobile } = useOperationSidebar()
const items = [{ title: 'İş Talepleri', path: '/operation-portal/work-requests', icon: ClipboardCheck }]
