  ...
  <section class="space-y-5">
    ...
    <template v-else-if="activeView === 'roles'">
      <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div v-for="role in roles" :key="role.id" class="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
          ...
          <div v-if="role.name !== 'super-admin'" class="mt-4 flex flex-wrap gap-1.5"><span v-for="permission in role.permissions" :key="permission.id" class="rounded-md border border-gray-100 bg-gray-50 px-2 py-1 text-[10px] text-gray-600 dark:border-gray-800 dark:bg-gray-800 dark:text-gray-300">{{ permission.name }}</span></div>
          <p v-else class="mt-4 text-xs text-gray-400">Sistem yöneticisi rolü — tüm yetkilere sahiptir.</p>
          <button v-if="role.name !== 'super-admin'" type="button" class="mt-4 text-xs font-medium text-brand-600 hover:underline" @click="selectRole(role.name); activeView = 'permissions'">Yetkileri düzenle →</button>
        </div>
      </div>
    </template>
    ...
    <template v-else>
      <div class="grid grid-cols-1 gap-5 lg:grid-cols-[260px_1fr]">
        <div class="rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-900"><p class="px-2 py-2 text-xs font-semibold text-gray-700 dark:text-gray-300">Roller</p><button v-for="role in roles.filter(role => role.name !== 'super-admin')" :key="role.id" type="button" class="mb-1 w-full rounded-lg px-3 py-2.5 text-left" :class="selectedRole === role.name ? 'bg-brand-50 dark:bg-brand-500/10' : 'hover:bg-gray-50 dark:hover:bg-gray-800'" @click="selectRole(role.name)"><p class="text-xs font-medium" :class="selectedRole === role.name ? 'text-brand-600 dark:text-brand-400' : 'text-gray-700 dark:text-gray-300'">{{ role.name }}</p><p class="mt-0.5 text-[10px] text-gray-400">{{ role.permissions?.length ?? 0 }} yetki</p></button></div>
