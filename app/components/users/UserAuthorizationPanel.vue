...
<template>
  <section class="space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <button type="button" class="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2 text-xs font-medium text-white hover:bg-brand-700" @click="showUserForm = true; selectedUser = null; userRole = ''; userPermissions = []">
        <UserRound :size="15" /> Yeni Kullanıcı Ekle
      </button>
    </div>
    ...
    <div v-if="showUserForm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" @click.self="closeUserAuthorization">
      <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
        <div class="flex items-start justify-between gap-3"><div><h2 class="text-base font-semibold text-gray-900">{{ selectedUser ? 'Kullanıcı Yetkileri' : 'Yeni Kullanıcı Ekle' }}</h2><p class="mt-1 text-xs text-gray-500">{{ selectedUser?.name ?? 'Yeni kullanıcı hesabı oluşturun.' }}</p></div><button type="button" class="text-gray-400 hover:text-gray-700" @click="closeUserAuthorization">×</button></div>
