...
<template>
  <section class="space-y-5">
    ...
    <template v-if="activeView==='users'">
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-3"><div class="rounded-xl border border-gray-200 bg-white p-4"><p class="text-xs text-gray-500">Toplam Kullanıcı</p><p class="mt-1 text-2xl font-semibold text-gray-900">{{users.length}}</p></div><div class="rounded-xl border border-gray-200 bg-white p-4"><p class="text-xs text-gray-500">Aktif</p><p class="mt-1 text-2xl font-semibold text-success-600">{{users.filter(u=>u.status==='Aktif').length}}</p></div><div class="rounded-xl border border-gray-200 bg-white p-4"><p class="text-xs text-gray-500">Rol Sayısı</p><p class="mt-1 text-2xl font-semibold text-brand-600">{{roles.length}}</p></div></div>
