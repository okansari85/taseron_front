...
const moduleConfig: Record<ModuleKey, ScopeKind[]> = {
  dashboard: ['tenant'], groups: ['tenant'], companies: ['group'], brands: ['group', 'company'], locations: ['group', 'company', 'brand'], hierarchy: ['group', 'company'],
}
...
export const useOrganizationScope = () => {
  ...
  const moduleKey = computed<ModuleKey>(() => { const path = route.path; if (path.includes('/organization/companies')) return 'companies'; if (path.includes('/organization/brands')) return 'brands'; if (path.includes('/organization/hierarchy')) return 'hierarchy'; if (path.includes('/organization/groups')) return 'groups'; if (path.includes('/locations')) return 'locations'; return 'dashboard' })
  const allowedKinds = computed(() => moduleConfig[moduleKey.value])
  const currentScope = computed(() => { if (!allowedKinds.value.includes(scope.value.kind)) return null; if (scope.value.kind === 'tenant') return currentTenant.value ? { id: currentTenant.value.id, name: currentTenant.value.name, description: 'Workspace kapsamı', icon: Building2 } : null; if (scope.value.kind === 'group') return scope.value.group; if (scope.value.kind === 'company') return scope.value.company; if (scope.value.kind === 'brand') return scope.value.brand; return scope.value.location })
  ...
  const optionsForKind = (kind: ScopeKind): ScopeOption[] => { if (kind === 'tenant') return currentTenant.value ? [{ id: currentTenant.value.id, name: currentTenant.value.name, description: 'Workspace kapsamı', icon: Building2 }] : []; if (kind === 'group') return groupOptions; if (kind === 'company') return scope.value.group ? (companyOptions[String(scope.value.group.id)] || []) : groupOptions.flatMap(group => companyOptions[String(group.id)] || []); if (kind === 'brand') return scope.value.company ? (brandOptions[String(scope.value.company.id)] || []) : []; if (kind === 'location') { if (scope.value.brand) return locationOptions[String(scope.value.brand.id)] || []; if (scope.value.company) return locationOptions[String(scope.value.company.id)] || []; if (scope.value.group) return locationOptions[String(scope.value.group.id)] || []; return [] } return [] }
  const setScope = (kind: ScopeKind, option: ScopeOption) => { if (kind === 'tenant') { scope.value = { kind, group: null, company: null, brand: null, location: null }; return }; if (kind === 'group') { scope.value = { kind, group: option, company: null, brand: null, location: null }; return }; if (kind === 'company') { const parentGroup = scope.value.group || groupOptions.find(group => (companyOptions[String(group.id)] || []).some(company => company.id === option.id)) || null; scope.value = { kind, group: parentGroup, company: option, brand: null, location: null }; return }; if (kind === 'brand') { scope.value = { kind, group: scope.value.group, company: scope.value.company, brand: option, location: null }; return }; scope.value = { ...scope.value, kind, location: option } }
  const resetForTenant = () => { scope.value = { kind: 'tenant', group: null, company: null, brand: null, location: null } }
  const breadcrumbs = computed(() => { const items: { kind: ScopeKind; id: string | number; name: string }[] = []; if (currentTenant.value) items.push({ kind: 'tenant', id: currentTenant.value.id, name: currentTenant.value.name }); if (scope.value.group) items.push({ kind: 'group', id: scope.value.group.id, name: scope.value.group.name }); if (scope.value.company) items.push({ kind: 'company', id: scope.value.company.id, name: scope.value.company.name }); if (scope.value.brand) items.push({ kind: 'brand', id: scope.value.brand.id, name: scope.value.brand.name }); if (scope.value.location) items.push({ kind: 'location', id: scope.value.location.id, name: scope.value.location.name }); return items })
  const goToBreadcrumb = (kind: ScopeKind) => { if (kind === 'tenant') return resetForTenant(); if (kind === 'group' && scope.value.group) return setScope('group', scope.value.group); if (kind === 'company' && scope.value.company) return setScope('company', scope.value.company); if (kind === 'brand' && scope.value.brand) return setScope('brand', scope.value.brand); if (kind === 'location' && scope.value.location) return setScope('location', scope.value.location) }
