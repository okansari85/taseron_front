...
const camelotResult = ref<Record<string, unknown> | null>(null)
const showSemantic = ref(true)
const showCamelot = ref(true)
...
const selectedSummary = computed(() => fixtures.value.find(item => item.fixture_id === selectedFixtureId.value) ?? null)
...
const runGemini = async () => {
  ...
  loading.value = true; fixture.value = null; v12Result.value = null; camelotResult.value = null
...
const runTemplateCamelot = async () => {
  if (!context.branchId) return $toast.error('Önce çalışma alanı/şube seçmelisin.')
  if (!selectedFixtureId.value) return $toast.error('Önce bir fixture seçmelisin.')
  if (selectedSummary.value && selectedSummary.value.pdf_available === false && !selectedFile.value) return $toast.error('Bu eski fixture için PDF kayıtlı değil. Aynı rapor PDF\'sini aşağıdan seçmelisin.')
  camelotLoading.value = true
  try { const response = await fireSuppressionReportApi.geminiFixtureCamelot(context.branchId, selectedFixtureId.value, selectedFile.value); camelotResult.value = response.data; showCamelot.value = true; $toast.success('Template + Camelot extraction tamamlandı.') }
  catch (error: any) { $toast.error(error?.data?.message || error?.message || 'Template + Camelot extraction başarısız oldu.') }
  finally { camelotLoading.value = false }
}
const runV12 = async () => {
  if (!context.branchId) return $toast.error('Önce çalışma alanı/şube seçmelisin.')
  if (!selectedFixtureId.value) return $toast.error('Önce bir fixture seçmelisin.')
  ...
  try { const response = await fireSuppressionReportApi.geminiFixtureV12(context.branchId, selectedFixtureId.value, selectedFile.value); v12Result.value = response.data; showV12.value = true; $toast.success('V12 analizi tamamlandı.') }
