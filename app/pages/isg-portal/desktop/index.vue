...
<template>
  <div v-if="context.ready" class="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <main class="px-5 py-6 sm:px-8 lg:px-10">
        <div class="mx-auto max-w-[1500px]">
          <div class="mb-7 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p class="mb-2 text-xs font-semibold uppercase tracking-[0.16em] text-brand-500">İSG Yönetim Paneli</p>
              <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">Güvenli İşletmeler Güçlü Gelecek</h1>
              <p class="mt-2 max-w-2xl text-sm leading-6 text-gray-500 dark:text-gray-400">
                İş sağlığı ve güvenliği süreçlerinizi tek merkezden yönetin.
              </p>
            </div>

            <div class="flex items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
              <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-50 text-brand-500 dark:bg-brand-500/10">
                <HardHat :size="18" />
              </div>
              <div class="min-w-0">
                <p class="text-[11px] font-medium uppercase tracking-wide text-gray-400">Aktif çalışma alanı</p>
                <p class="truncate text-sm font-semibold text-gray-800 dark:text-white/90">
                  {{ context.locationName }}<span v-if="context.branchName" class="font-normal text-gray-400"> / {{ context.branchName }}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <NuxtLink
              ...
              class="group rounded-2xl border border-gray-200 bg-white p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-sm dark:border-gray-800 dark:bg-gray-900 dark:hover:border-brand-500/30"
            >
              <div class="flex items-start justify-between gap-4">
                <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-gray-50 text-brand-500 dark:bg-gray-800">
              ...
              <p class="mt-5 text-[10px] font-bold tracking-[0.14em] text-gray-400">{{ module.eyebrow }}</p>
          ...
          <section class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(320px,1fr)]">
            <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
              ...
              <div class="grid gap-3 p-5 sm:grid-cols-2">
                <NuxtLink to="/isg-portal/documents" class="flex items-center justify-between rounded-xl border border-gray-100 px-4 py-4 transition-colors hover:border-brand-200 hover:bg-brand-50/40 dark:border-gray-800 dark:hover:border-brand-500/30 dark:hover:bg-brand-500/5">
                  <div>
                ...
                <NuxtLink to="/isg-portal/fire-inspection" class="flex items-center justify-between rounded-xl border border-gray-100 px-4 py-4 transition-colors hover:border-brand-200 hover:bg-brand-50/40 dark:border-gray-800 dark:hover:border-brand-500/30 dark:hover:bg-brand-500/5">
                  <div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
              <div class="flex items-start gap-4">
                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand-500 dark:bg-brand-500/10">
                  <HardHat :size="19" />
                </div>
                <div>
                  <h2 class="text-sm font-bold text-gray-900 dark:text-white">Çalışma Alanı</h2>
                  <p class="mt-1 text-xs leading-5 text-gray-400">Şu anda bu lokasyon ve şube için çalışıyorsunuz.</p>
                </div>
              </div>
              <div class="mt-6 border-t border-gray-100 pt-5 dark:border-gray-800">
                <p class="text-xs font-medium text-gray-400">Lokasyon</p>
                <p class="mt-1 text-sm font-semibold text-gray-800 dark:text-white/90">{{ context.locationName }}</p>
                <p v-if="context.branchName" class="mt-3 text-xs font-medium text-gray-400">Şube</p>
                <p v-if="context.branchName" class="mt-1 text-sm font-semibold text-gray-800 dark:text-white/90">{{ context.branchName }}</p>
              </div>
            </div>
          </section>
        </div>
