  ...
  <div v-if="context.ready" class="min-h-screen bg-[#f7f8fa] font-outfit text-gray-900 dark:bg-gray-950 dark:text-white">
    ...
    <div :class="['min-h-screen transition-[padding] duration-300', isExpanded ? 'lg:pl-[230px]' : 'lg:pl-[72px]']">
      ...
      <main class="flex flex-col items-center justify-center px-5 py-24 text-center">
        <span class="flex h-14 w-14 items-center justify-center rounded-xl bg-gray-100 text-gray-500 dark:bg-white/5"><HelpCircle :size="26" /></span>
        <h1 class="mt-5 text-lg font-bold text-[#172033] dark:text-white">Destek</h1>
        <p class="mt-2 max-w-sm text-sm text-gray-500 dark:text-gray-400">Bu bölüm hazırlanıyor.</p>
      </main>
    </div>
  </div>
