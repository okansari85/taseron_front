...
const systems = [
  {
    ...
    description: '2 ana + 1 jokey pompa',
    status: 'Uygunsuzluk Var',
    statusClass: 'danger',
    controls: 14,
    defects: 12,
    icon: Gauge,
  ...
  {
    name: 'Sprinkler Sistemi',
    description: 'Upright, Pendent',
    status: 'Uygun',
    statusClass: 'success',
    controls: 12,
    defects: 0,
  ...
  <div class="fire-status-page">
    ...
    <section class="summary-card">
      <div class="summary-main">
        <div class="summary-icon summary-danger-icon"><Flame :size="29" /></div>
        <div>
          <span class="summary-label">Genel Durum</span>
          <strong class="summary-danger">Uygun Değil</strong>
          <p>Son rapora göre yangın tesisatında uygunsuzluklar bulunmaktadır.</p>
        </div>
      </div>
      ...
      <div class="summary-item">
        <div class="small-icon blue"><CalendarDays :size="22" /></div>
        <div>
          <span class="summary-label">Son Periyodik Kontrol</span>
          <strong>10.04.2026</strong>
          <small>4A Periyodik Kontrol</small>
        </div>
      </div>
      ...
      <div class="summary-item">
        <div class="small-icon green"><ShieldCheck :size="22" /></div>
        <div>
          <span class="summary-label">Geçerlilik Tarihi</span>
          <strong>10.04.2027</strong>
          <small class="green-text">Kalan süre: 218 gün</small>
        </div>
      </div>
      ...
      <div class="summary-item">
        <div class="small-icon gray"><FileText :size="22" /></div>
        <div>
          <span class="summary-label">Rapor No</span>
          <strong>PK.239.00026.01</strong>
          <button class="report-link" type="button">Raporu Görüntüle <ArrowRight :size="14" /></button>
        </div>
      </div>
    </section>
    ...
    <template v-if="activeTab === 'Sistemler'">
      <div class="section-heading">
        <div>
          <h2>Sistem Bazlı Durum</h2>
          <p>Son rapora göre yangın tesisatında bulunan sistemlerin kontrol sonuçları.</p>
        </div>

        <div class="view-switcher">
          <button :class="{ active: viewMode === 'cards' }" type="button" @click="viewMode = 'cards'">
            <LayoutGrid :size="14" /> Kart Görünümü
          </button>
      ...
      <div v-if="viewMode === 'cards'" class="systems-grid">
        <article v-for="system in systems" :key="system.name" class="system-card">
          <div :class="['system-icon', `system-${system.statusClass}`]">
            <component :is="system.icon" :size="25" />
          </div>
          ...
          <div class="system-content">
            ...
            <template v-if="system.progress">
              <div class="progress-track"><div class="progress-fill" :style="{ width: `${system.progress}%` }" /></div>
              <div class="progress-label">20 / 20 uygunsuz</div>
            </template>

            <div class="system-meta">
              <span>{{ system.controls }} kontrol maddesi</span>
              <span>{{ system.defects }} uygunsuzluk{{ system.defects === 1 ? '' : '' }}</span>
            </div>
          </div>
        </article>
      </div>
      ...
      <div class="lower-grid">
        <section class="panel">
          <div class="panel-title"><h3>Kontrol Sonuçlarının Dağılımı</h3><BarChart3 :size="16" /></div>
          <div class="donut-area">
            <div class="donut"><div class="donut-center"><strong>44</strong><span>Kontrol Maddesi</span></div></div>
            <div class="legend">
              <div><span class="legend-dot success-dot" /> <span>Uygun</span><b>24 (%55)</b></div>
              <div><span class="legend-dot danger-dot" /> <span>Uygun Değil</span><b>16 (%36)</b></div>
              <div><span class="legend-dot neutral-dot" /> <span>Uygulaması Yok</span><b>4 (%9)</b></div>
            </div>
          </div>
        </section>

        <section class="panel">
          <div class="panel-title"><h3>Sistemlere Göre Uygunsuzluk Sayısı</h3><ClipboardCheck :size="16" /></div>
          <div class="bar-list">
            <div v-for="item in defectStats" :key="item.name" class="bar-row">
              <span>{{ item.name }}</span>
              <div class="bar-track"><div class="bar-fill" :style="{ width: `${(item.value / 6) * 100}%` }" /></div>
        ...
        <section class="panel">
          <div class="panel-title"><h3>Tesisat Bilgileri (Rapor'dan)</h3><FileText :size="16" /></div>
          <div class="facility-list">
            <div v-for="item in facilityInfo" :key="item.label" class="facility-row">
