...
const login = async () => {
  ...
  try {
    ...
    if (!roles.includes('isg')) {
      await auth.logout()
      errorMessage.value = 'Bu panel yalnızca İSG kullanıcıları içindir.'
      return
    }

    if (!response.user.tenant_id) {
      await auth.logout()
      errorMessage.value = 'Bu kullanıcıya henüz bir tenant ataması yapılmamış. Lütfen yönetici ile iletişime geçin.'
      return
    }
  ...
  <div class="min-h-screen bg-gray-50 px-4 py-10 dark:bg-gray-950">
    <div class="mx-auto flex min-h-[80vh] max-w-md items-center">
      <div class="w-full rounded-2xl border border-gray-200 bg-white p-8 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
        ...
        <h1 class="mt-5 text-center text-2xl font-semibold">İSG Portalı</h1>
        ...
        <div class="mt-6 rounded-xl bg-gray-50 p-3 text-xs text-gray-500 dark:bg-white/5">
          Bu panel yalnızca İSG uzmanı kullanıcıları içindir.
        </div>
      </div>
    </div>
  </div>
