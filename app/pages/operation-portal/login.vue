...
import { LockKeyhole, ClipboardCheck } from '@lucide/vue'
...
const login = async () => {
  ...
  try {
    ...
    if (!roles.includes('operation')) {
      await auth.logout()
      errorMessage.value = 'Bu panel yalnızca operasyon kullanıcıları içindir.'
      return
    }

    if (!response.user.tenant_id) {
      await auth.logout()
      errorMessage.value = 'Bu kullanıcıya henüz bir tenant ataması yapılmamış. Lütfen yonetici ile iletisime gecin.'
      return
    }

    tenantContext.setTenantId(response.user.tenant_id)
    await navigateTo('/operation-portal/work-requests')
  } catch (error: any) {
    ...
    if (status === 401) {
      ...
      errorMessage.value = 'Sunucuya baglanilamadi.'
    } else {
      errorMessage.value = error?.data?.message || error?.message || 'Giris sirasinda bir hata olustu.'
    }
  } finally {
  ...
  <div class="min-h-screen bg-gray-50 px-4 py-10 dark:bg-gray-950">
    <div class="mx-auto flex min-h-[80vh] max-w-md items-center">
      <div class="w-full rounded-2xl border border-gray-200 bg-white p-8 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
        ...
        <h1 class="mt-5 text-center text-2xl font-semibold">Operasyon Portalı</h1>
        <p class="mt-1 text-center text-sm text-gray-500">E-posta ve sifrenizle giris yapin.</p>
        <form class="mt-8 space-y-4" @submit.prevent="login">
          ...
          <label class="block">
            <span class="mb-2 block text-sm">Şifrg</span>
            <div class="relative">
        ...
        <div class="mt-6 rounded-xl bg-gray-50 p-3 text-xs text-gray-500 dark:bg-white/5">
          Bu panel yalnizza operasyon kullanicilari icindir.
        </div>
      </div>
    </div>
  </div>
