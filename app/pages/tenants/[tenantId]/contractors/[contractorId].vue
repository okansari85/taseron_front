...
const contractorId = computed(() =>
  Number(
    Array.isArray(route.params.contractorId)
      ? route.params.contractorId[0]
      : (route.params.contractorId ?? 1),
  ),
);
type Contractor = {
  id: number;
...
const contractor = computed(
  () => contractors.find((x) => x.id === contractorId.value) ?? contractors[0],
);
const isTemporary = computed(
  () =>
    contractor.value.type === "Geçici" ||
    [4, 5, 7, 9, 12].includes(contractorId.value),
);
const statusText = computed(() =>
  contractor.value.status === "active" ? "Aktif" : "Pasif",
);
...
const tabs = computed(() => [
  ...
  ...(isTemporary.value
    ? [{ key: "visits", label: "Saha Ziyaretleri", icon: History }]
    : [
...
const locations = [
  { name: "Beylikdüzü Kampüsü", business: "Arçelik A.Ş.", status: "Aktif" },
  {
    ...
    business: "Arçelik Pazarlama A.Ş.",
    status: "Aktif",
  },
...
const visits = [
  {
    ...
    business: "Arçelik A.Ş.",
    purpose: "Planlı ekipman kontrolü",
    people: 4,
    status: "Planlanan",
  },
  {
    ...
    business: "Arçelik A.Ş.",
    purpose: "Teknik bakım ve saha çalışması",
    people: 3,
    status: "Devam Ediyor",
  },
  {
    ...
    location: "Beylikdüzü Kampüsü",
    business: "Arçelik A.Ş.",
    purpose: "Forklift bakım ve teknik servis",
    people: 3,
    status: "Tamamlandı",
  },
  {
  ...
  },
  {
    ...
    location: "Beylikdüzü Kampüsü",
    business: "Arçelik A.Ş.",
    purpose: "Makine arıza müdahalesi",
    people: 2,
    status: "Tamamlandı",
  },
];
const tabContractor = computed(() => ({
  ...contractor.value,
...
<template>
  <div class="font-outfit mx-auto w-full max-w-[1400px]">
    <div class="mb-6 flex items-start gap-4">
      ...
      <div>
        ...
        <p class="mt-1.5 text-sm text-gray-500">
          Alt yüklenici detayları, organizasyon ilişkileri ve saha geçmişi.
        </p>
      </div>
    </div>
    <div class="mb-5 grid grid-cols-1 gap-4 lg:grid-cols-4">
      <section
        class="rounded-xl border border-gray-200 bg-white p-5 shadow-theme-xs lg:col-span-3"
      >
        <div class="flex items-center gap-4">
          <div
            class="flex h-12 w-12 items-center justify-center rounded-full bg-brand-50 text-sm font-bold text-brand-600"
          >
            {{ contractor.shortName.slice(0, 2).toUpperCase() }}
          </div>
          <div>
            <p class="text-xs text-gray-500">Genel değerlendirme</p>
            <div class="mt-1 flex items-center gap-2">
              ...
              <div class="flex items-center gap-0.5">
                <Star
                  ...
                  :size="16"
                  :class="
                    n <= Math.round(contractor.rating)
            ...
            <p class="mt-1 text-xs text-gray-500">
              {{ contractor.reviewCount.toLocaleString("tr-TR") }} değerlendirme
            </p>
          </div>
          <div class="ml-auto hidden text-right sm:block">
            <p class="text-xs text-gray-500">Son değerlendirme</p>
            <p class="mt-1 text-sm font-medium text-gray-800">4,5 / 5</p>
            <p class="mt-1 text-[11px] text-gray-400">Son 12 ay</p>
          </div>
        </div>
      </section>
      <section
        class="rounded-xl border border-gray-200 bg-white p-5 shadow-theme-xs"
      >
        <p class="text-xs text-gray-500">Durum</p>
        <div class="mt-2 flex items-center gap-2">
          <CheckCircle2 :size="18" class="text-success-500" /><span
            class="text-sm font-semibold"
            >{{ statusText }}</span
          >
        </div>
        <p class="mt-3 text-xs text-gray-500">
          {{ organizations.length }} organizasyon eşleşmesi
        </p>
      </section>
    </div>
    <div
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-sm font-semibold text-amber-800">
            Geçici Alt Yüklenici
          </p>
          <p class="mt-1 text-xs text-amber-700/80">
            Fiili saha ziyaretleri bu firma için ayrıca takip edilir.
          </p>
        </div>
      </div>
      <NuxtLink
        :to="temporaryHistoryUrl"
        class="rounded-lg bg-amber-500 px-3 py-2 text-xs font-semibold text-white"
        >Tüm Ziyaretleri Gör</NuxtLink
      >
    </div>
    <div
      class="mb-5 overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-theme-xs"
    >
    ...
    <ContractorVisitsTab
      ...
      :visits="visits"
      :history-url="temporaryHistoryUrl"
    />
  </div>
</template>
