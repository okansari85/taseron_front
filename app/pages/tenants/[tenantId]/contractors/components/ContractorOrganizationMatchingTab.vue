...
type Contractor = { id: number; name: string; shortName: string; type: string; organizations: string[] };
type Organization = { id: number; name: string; type: string; parent: string };
...
const selectedOrganizations = ref<number[]>([2]);

const contractors: Contractor[] = [
  { id: 1, name: "ISS Tesis Yönetim Hizmetleri A.Ş.", shortName: "ISS", type: "Daimi", organizations: ["Dayanıklı Tüketim Grubu", "Otomotiv Grubu", "Enerji Grubu"] },
  { id: 2, name: "Tepe Savunma ve Güvenlik Sistemleri A.Ş.", shortName: "Tepe", type: "Daimi", organizations: ["Otomotiv Grubu"] },
  { id: 3, name: "Sofra Yemek Üretim ve Hizmet A.Ş.", shortName: "Sofra", type: "Daimi", organizations: ["Dayanıklı Tüketim Grubu"] },
  { id: 4, name: "Bantaş Temizlik Hizmetleri Ltd. Şti.", shortName: "Bantaş", type: "Geçici", organizations: [] },
  { id: 5, name: "ABC Teknik Bakım Hizmetleri Ltd. Şti.", shortName: "ABC Teknik", type: "Geçici", organizations: ["Enerji Grubu", "Turizm Grubu"] },
  { id: 6, name: "Protek Güvenlik Hizmetleri A.Ş.", shortName: "Protek", type: "Daimi", organizations: ["Koç Holding", "Otomotiv Grubu"] },
  { id: 7, name: "CleanPro Endüstriyel Temizlik A.Ş.", shortName: "CleanPro", type: "Geçici", organizations: [] },
  { id: 8, name: "Güven Plus Özel Güvenlik Ltd. Şti.", shortName: "Güven Plus", type: "Daimi", organizations: ["Dayanıklı Tüketim Grubu"] },
  { id: 9, name: "Lezzet Catering Hizmetleri A.Ş.", shortName: "Lezzet", type: "Geçici", organizations: ["Turizm Grubu"] },
  { id: 10, name: "TeknikServis Bakım ve Onarım A.Ş.", shortName: "TeknikServis", type: "Daimi", organizations: ["Enerji Grubu"] },
];
...
const filteredContractors = computed(() => {
  ...
  return contractors.filter((x) => !t || `${x.name} ${x.shortName} ${x.type}`.toLocaleLowerCase("tr-TR").includes(t));
});
const filteredOrganizations = computed(() => {
  const t = organizationSearch.value.trim().toLocaleLowerCase("tr-TR");
  return organizations.filter((x) => !t || `${x.name} ${x.type}`.toLocaleLowerCase("tr-TR").includes(t));
});
const toggleContractor = (id: number) => {
  selectedContractors.value = selectedContractors.value.includes(id) ? selectedContractors.value.filter((x) => x !== id) : [...selectedContractors.value, id];
};
const toggleOrganization = (id: number) => {
  selectedOrganizations.value = selectedOrganizations.value.includes(id) ? selectedOrganizations.value.filter((x) => x !== id) : [...selectedOrganizations.value, id];
};
  ...
  <section class="rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1.75fr)_minmax(260px,0.65fr)]">
      <div class="border-b border-gray-200 p-5 dark:border-gray-800 lg:border-b-0 lg:border-r">
        <div class="mb-4 flex items-center justify-between gap-3"><div><h2 class="text-sm font-semibold text-gray-900 dark:text-white/90">Alt Yükleniciler</h2><p class="mt-1 text-xs text-gray-500">{{ selectedContractors.length }} alt yüklenici seçildi</p></div><div class="flex gap-2"><button type="button" class="text-xs font-medium text-brand-500" @click="selectAllContractors">Tümünü seç</button><button type="button" class="text-xs font-medium text-gray-500" @click="clearContractors">Temizle</button></div></div>
        ...
        <div class="max-h-[500px] overflow-y-auto rounded-lg border border-gray-100 dark:border-gray-800">
          <button v-for="item in filteredContractors" :key="item.id" type="button" class="flex w-full items-center gap-3 border-b border-gray-100 px-3 py-3 text-left last:border-b-0 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/[0.02]" @click="toggleContractor(item.id)">
            ...
            <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-50 text-xs font-bold text-brand-600 dark:bg-brand-500/10">{{ item.shortName.slice(0, 2).toUpperCase() }}</span>
            <span class="min-w-0 flex-1"><span class="block truncate text-sm font-medium text-gray-800 dark:text-white/90">{{ item.name }}</span><span class="mt-0.5 flex min-w-0 flex-wrap items-center gap-1.5 text-xs text-gray-500"><span>{{ item.shortName }} · {{ item.type }}</span><template v-if="item.organizations.length"><span v-for="name in item.organizations.slice(0, 3)" :key="name" class="rounded-full bg-brand-50 px-2 py-0.5 font-medium text-brand-600 dark:bg-brand-500/10">{{ name }}</span><span v-if="item.organizations.length > 3" class="rounded-full bg-gray-100 px-2 py-0.5 font-medium text-gray-500 dark:bg-gray-800">+{{ item.organizations.length - 3 }}</span></template><span v-else class="rounded-full bg-gray-100 px-2 py-0.5 font-medium text-gray-500 dark:bg-gray-800">Eşleşmemiş</span></span></span>
          </button>
        </div>
      </div>
      <div class="p-5">
        ...
        <div class="space-y-2"><button v-for="item in filteredOrganizations" :key="item.id" type="button" class="flex w-full items-center gap-2.5 rounded-lg border p-2.5 text-left transition" :class="selectedOrganizations.includes(item.id) ? 'border-brand-500 bg-brand-50/60 dark:border-brand-500 dark:bg-brand-500/10' : 'border-gray-200 hover:border-gray-300 dark:border-gray-700'" @click="toggleOrganization(item.id)"><span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-500 dark:bg-gray-800"><Building2 :size="15" /></span><span class="min-w-0 flex-1"><span class="block truncate text-sm font-medium text-gray-800 dark:text-white/90">{{ item.name }}</span><span class="mt-0.5 block truncate text-[11px] text-gray-500">{{ item.parent ? `${item.parent} · ` : "" }}{{ item.type }}</span></span><span class="flex h-5 w-5 shrink-0 items-center justify-center rounded border" :class="selectedOrganizations.includes(item.id) ? 'border-brand-500 bg-brand-500 text-white' : 'border-gray-300 text-transparent dark:border-gray-600'"><Check :size="12" /></span></button></div>
