...
import { ArrowRight, Building2, Check, Search, X } from "lucide-vue-next";
...
const filteredOrganizations = computed(() => {
  ...
  return organizations.value.filter((x) => !t || `${x.name} ${x.type}`.toLocaleLowerCase("tr-TR").includes(t));
});
...
const toggleContractor = (id: number) => { selectedContractors.value = selectedContractors.value.includes(id) ? selectedContractors.value.filter((x) => x !== id) : [...selectedContractors.value, id]; };
const toggleOrganization = (id: number) => { selectedOrganizations.value = selectedOrganizations.value.includes(id) ? selectedOrganizations.value.filter((x) => x !== id) : [...selectedOrganizations.value, id]; };
      ...
      <div class="border-b border-gray-200 p-5 dark:border-gray-800 lg:border-b-0 lg:border-r">
        ...
        <div class="max-h-[500px] overflow-y-auto rounded-lg border border-gray-100 dark:border-gray-800">
          ...
          <button v-for="item in filteredContractors" :key="item.id" type="button" class="flex w-full items-center gap-3 border-b border-gray-100 px-3 py-3 text-left last:border-b-0 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/[0.02]" @click="toggleContractor(item.id)">
            ...
            <span class="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-brand-50 text-xs font-bold text-brand-600 dark:bg-brand-500/10">
              <img v-if="item.contractor?.logo_path && !logoErrors[item.id]" :src="logoUrl(item.contractor.logo_path)" :alt="`${item.name} logosu`" class="h-full w-full bg-white object-contain" @error="onLogoError(item.id)" />
              <template v-else>{{ contractorShortName(item).slice(0, 2).toUpperCase() }}</template>
            </span>
            <span class="min-w-0 flex-1"><span class="block truncate text-sm font-medium text-gray-800 dark:text-white/90">{{ item.name }}</span><span class="mt-0.5 flex min-w-0 flex-wrap items-center gap-1.5 text-xs text-gray-500"><span>{{ contractorShortName(item) }} · {{ contractorType(item) }}</span><template v-if="item.organizations?.length"><span v-for="organization in item.organizations.slice(0, 3)" :key="organization.id" class="inline-flex items-center gap-1 rounded-full bg-brand-50 pl-2 pr-1 py-0.5 font-medium text-brand-600 dark:bg-brand-500/10">{{ organization.name }}<span role="button" tabindex="0" class="inline-flex h-4 w-4 items-center justify-center rounded-full hover:bg-brand-100" title="Eşleştirmeyi kaldır" @click.stop="removeMatching(organization.id, item.id)"><X :size="10" /></span></span><span v-if="item.organizations.length > 3" class="rounded-full bg-gray-100 px-2 py-0.5 font-medium text-gray-500 dark:bg-gray-800">+{{ item.organizations.length - 3 }}</span></template><span v-else class="rounded-full bg-gray-100 px-2 py-0.5 font-medium text-gray-500 dark:bg-gray-800">Eşleşmemiş</span></span></span>
          </button>
