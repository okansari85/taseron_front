...
import type { Contractor } from "~/data/contractors.mock";
...
const mapContractor = (record: ContractorApiRecord): Contractor => {
  ...
  return { id: record.id, name, shortName, type: record.contractor_type === "permanent" ? "Daimi" : "Geçici", status: "active", initials, avatarClass: "bg-brand-50 text-brand-500" };
};
...
const loadContractors = async () => {
  ...
  listError.value = "";
  try {
    const records = await contractorApi.list();
    contractors.value = records.map(mapContractor);
  } catch (error) {
    listError.value = error instanceof Error ? error.message : "Alt yükleniciler alınamadı.";
  } finally {
    listLoading.value = false;
  }
};

const addContractor = async (payload: { name: string; shortName: string; type: "Daimi" | "Geçici"; status: "active" | "passive" }) => {
  createLoading.value = true;
  createError.value = "";
  try {
    const created = await contractorApi.create({ name: payload.name, contractor_type: payload.type === "Daimi" ? "permanent" : "temporary" });
    contractors.value.unshift(mapContractor(created));
...
<template>
  <div class="col-span-full block w-full min-w-0" style="display: block !important; width: 100% !important; grid-column: 1 / -1 !important;">
    <div v-if="listError" class="mb-4 rounded-lg border border-error-200 bg-error-50 px-4 py-3 text-sm text-error-600">{{ listError }}</div>
