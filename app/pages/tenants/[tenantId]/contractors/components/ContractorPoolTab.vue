...
const removeContractor = async (contractor: Contractor) => {
  if (!window.confirm(`“${contractor.name}” alt yüklenicisi silinsin mi?`)) return;
  createError.value = "";
  try {
    await contractorStore.deleteContractor(contractor.id);
