...
const props = defineProps<{ tenantId: string; fixedType?: "Daimi" | "Geçici" }>();
const contractorStore = useContractorStore();
const { contractors } = storeToRefs(contractorStore);
const search = ref("");
const typeFilter = ref(props.fixedType ?? "all");
...
const effectiveType = computed(() => props.fixedType ?? typeFilter.value);
const filtered = computed(() => {
  ...
  return contractors.value.filter((x) =>
    ...
    (effectiveType.value === "all" || x.type === effectiveType.value) &&
...
const resetFilters = () => { search.value = ""; typeFilter.value = props.fixedType ?? "all"; statusFilter.value = "all"; };
...
watch(() => props.tenantId, loadContractors, { immediate: true });
</script>
...
<template>
  <div class="col-span-full block w-full min-w-0" style="display:block!important;width:100%!important;grid-column:1/-1!important;">
    ...
    <ContractorCreateDrawer v-model="drawerOpen" :edit-data="editingContractor" :default-type="props.fixedType ?? 'Daimi'" @save="saveContractor" />
    <ConfirmationModal
