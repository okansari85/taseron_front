...
import { Building2, ChevronDown, Search, Users, ArrowRight } from 'lucide-vue-next'
import OrganizationTabs from '~/components/organization/OrganizationTabs.vue'

definePageMeta({ layout: 'default' })

const contractorTabs = [
  { label: 'Alt Yüklenici Havuzu', path: 'index' },
...
const selectAll = () => {
  selectedContractors.value = filteredContractors.value.map(item => item.id)
}
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[1fr_360px]">
        <div class="border-b border-gray-200 p-5 dark:border-gray-800 lg:border-b-0 lg:border-r">
          <div class="mb-4 flex items-center justify-between gap-3">
            <div><h2 class="text-sm font-semibold text-gray-900 dark:text-white/90">Alt Yükleniciler</h2><p class="mt-1 text-xs text-gray-500">{{ selectedContractors.length }} kayıt seçildi</p></div>
            <div class="flex gap-2"><button type="button" class="text-xs font-medium text-brand-500" @click="selectAll">Tümünü seç</button><button type="button" class="text-xs font-medium text-gray-500" @click="clearSelection">Temizle</button></div>
          </div>
          ...
          <div class="max-h-[520px] overflow-y-auto rounded-lg border border-gray-100 dark:border-gray-800">
            <button v-for="item in filteredContractors" :key="item.id" type="button" class="flex w-full items-center gap-3 border-b border-gray-100 px-3 py-3 text-left last:border-b-0 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/[0.02]" @click="toggleContractor(item.id)">
              <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded border text-xs" :class="selectedContractors.includes(item.id) ? 'border-brand-500 bg-brand-500 text-white' : 'border-gray-300 text-transparent dark:border-gray-600'">✓</span>
              ...
              <span class="min-w-0 flex-1"><span class="block truncate text-sm font-medium text-gray-800 dark:text-white/90">{{ item.name }}</span><span class="mt-0.5 block text-xs text-gray-500">{{ item.shortName }} · {{ item.type }}</span></span>
              <span v-if="item.assigned" class="rounded-full bg-success-50 px-2 py-1 text-[10px] font-semibold text-success-600">Eşleşmiş</span>
            </button>
          </div>
        </div>

        <div class="p-5">
          <div class="mb-4"><h2 class="text-sm font-semibold text-gray-900 dark:text-white/90">Organizasyon</h2><p class="mt-1 text-xs text-gray-500">Seçilen alt yüklenicilerin atanacağı düğümü seçin.</p></div>
          ...
          <div class="space-y-2">
            <button v-for="item in filteredOrganizations" :key="item.id" type="button" class="flex w-full items-center gap-3 rounded-lg border p-3 text-left transition" :class="selectedOrganization === item.id ? 'border-brand-500 bg-brand-50/60 dark:border-brand-500 dark:bg-brand-500/10' : 'border-gray-200 hover:border-gray-300 dark:border-gray-700'" @click="selectedOrganization = item.id">
              <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-gray-100 text-gray-500 dark:bg-gray-800"><Building2 :size="16" /></span>
              <span class="min-w-0 flex-1"><span class="block text-sm font-medium text-gray-800 dark:text-white/90">{{ item.name }}</span><span class="mt-0.5 block text-xs text-gray-500">{{ item.parent ? `${item.parent} · ` : '' }}{{ item.type }}</span></span>
              <span v-if="selectedOrganization === item.id" class="h-2 w-2 rounded-full bg-brand-500" />
            </button>
          </div>
          <div class="mt-5 rounded-lg bg-gray-50 p-4 dark:bg-white/[0.03]"><div class="flex items-center justify-between text-xs text-gray-500"><span>{{ selectedContractors.length }} alt yüklenici</span><ArrowRight :size="15" /><span class="font-medium text-gray-700 dark:text-gray-300">{{ organizations.find(item => item.id === selectedOrganization)?.name }}</span></div></div>
          <button type="button" class="mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 text-sm font-semibold text-white shadow-theme-xs hover:bg-brand-600" :disabled="selectedContractors.length === 0"><ArrowRight :size="16" /> Seçilenleri Eşleştir</button>
        </div>
      </div>
    </section>
  </div>
