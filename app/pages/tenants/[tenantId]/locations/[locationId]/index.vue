...
const filteredItems = computed(() => activeTab.value === "companies" ? filteredCompanies.value : filteredContractors.value);
...
const operationalAreasTab = ref<InstanceType<typeof LocationOperationalAreasTab> | null>(null);
...
<template>
  <div class="font-outfit mx-auto w-full max-w-[1400px]">
    ...
    <div class="mb-4 flex items-center justify-end">
      ...
      <button v-else class="inline-flex h-10 items-center gap-2 rounded-lg bg-brand-500 px-4 text-sm font-semibold text-white" @click="operationalAreasTab?.openCreate()"><Plus :size="16" /> Operasyonel Alan Ekle</button>
    </div>

    <section v-if="activeTab!=='areas'" class="mb-5 rounded-xl border border-gray-200 bg-white p-4 shadow-theme-xs">
      <div class="grid grid-cols-1 gap-3 md:grid-cols-[minmax(220px,1.2fr)_1fr_auto] md:items-center">
        <div class="relative"><input v-model="search" type="search" :placeholder="activeTab==='companies'?'Firma ara...':'Alt yüklenici ara...'" class="h-11 w-full rounded-lg border border-gray-200 bg-white px-3 text-sm outline-none" /></div>
    ...
    <LocationOperationalAreasTab v-if="activeTab==='areas'" ref="operationalAreasTab" :location-id="locationId" />
