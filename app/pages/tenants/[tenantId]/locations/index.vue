...
const additionalLocations = mockKocLocations.filter(item => item.id > 9).map((item, index): Location => ({
  id: item.id,
  name: item.name,
  city: item.city,
  district: item.district,
  address: item.address,
  businessEntities: [{ id: 1000 + item.id, companyName: item.companyName, brandName: item.brandName, logoUrl: remoteLogo(item.brandName || '', item.companyName) || item.logoUrl }],
  contractorCount: item.contractorCount,
  status: item.status,
  image: remoteImage(index),
}))
...
const filteredLocations = computed(() => {
  ...
  return locations.value.filter(location => {
    ...
    return (!term || haystack.includes(term)) && (companyFilter.value === 'all' || location.businessEntities.some(entity => entity.companyName === companyFilter.value)) && (cityFilter.value === 'all' || location.city === cityFilter.value) && (statusFilter.value === 'all' || location.status === statusFilter.value)
  })
})
...
<template>
  <div class="font-outfit"><div class="mx-auto w-full max-w-[1400px]">
    ...
    <section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]"><div class="overflow-x-auto"><table class="w-full min-w-[1180px] text-left"><thead class="border-b border-gray-100 bg-gray-50/70 dark:border-gray-800 dark:bg-white/[0.03]"><tr><th class="px-4 py-4 text-xs font-medium text-gray-500 dark:text-gray-400">Lokasyon</th><th class="px-4 py-4 text-xs font-medium text-gray-500 dark:text-gray-400">Adres</th><th class="px-4 py-4 text-xs font-medium text-gray-500 dark:text-gray-400">Şirketler / Markalar</th><th class="px-4 py-4 text-center text-xs font-medium text-gray-500 dark:text-gray-400">Taşeron</th><th class="px-4 py-4 text-xs font-medium text-gray-500 dark:text-gray-400">Durum</th><th class="px-4 py-4 text-right text-xs font-medium text-gray-500 dark:text-gray-400">İşlemler</th></tr></thead>
      <tbody class="divide-y divide-gray-100 dark:divide-gray-800"><tr v-for="location in paginatedLocations" :key="location.id" class="transition hover:bg-gray-50/70 dark:hover:bg-white/[0.02]">
