...
import { ArrowLeft, Check, ChevronDown, FileImage, LockKeyhole, Save, X } from 'lucide-vue-next'
...
const selectedCompany = computed(() => companies.value.find(company => company.id === companyId.value))
...
const save = async () => {
  if (!brandName.value.trim() || !shortName.value.trim() || companyId.value === null) {
    errorMessage.value = 'Marka adı, kısa ad ve şirket seçimi zorunludur.'
    return
  }

  errorMessage.value = ''
  try {
    await brandStore.createBrand({ name: brandName.value.trim(), shortName: shortName.value.trim(), description: description.value.trim(), isActive: status.value === 'active', companyIds: [companyId.value], logo: logoFile.value })
    await goBack()
  } catch (error: any) {
...
<template>
  <div class="font-outfit"><div class="mx-auto w-full max-w-[1400px]">
    ...
    </div><div class="flex items-center justify-end gap-3 border-t border-gray-100 px-5 py-4 dark:border-gray-800"><button type="button" class="h-10 rounded-lg border border-gray-200 bg-white px-5 text-sm font-medium text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400" @click="goBack">İptal</button><button type="button" :disabled="saving" class="inline-flex h-10 items-center gap-2 rounded-lg bg-brand-500 px-5 text-sm font-semibold text-white hover:bg-brand-600 disabled:opacity-60" @click="save"><Save :size="16" />{{ saving ? 'Kaydediliyor...' : 'Kaydet' }}</button></div></section>
  </div></div>
</template>
