...
const filteredCompanies = computed(() => {
  ...
  return companies.value.filter((company) => {
    ...
    return (
      matchesSearch &&
      (groupFilter.value === "all" || company.group === groupFilter.value) &&
      (statusFilter.value === "all" || company.status === statusFilter.value)
    );
  });
});
const totalPages = computed(() =>
  Math.max(1, Math.ceil(filteredCompanies.value.length / perPage.value)),
);
...
watch([search, groupFilter, statusFilter, perPage], () => {
  currentPage.value = 1;
});
const resetFilters = () => {
  search.value = "";
...
const goToCreate = () =>
  navigateTo(`/tenants/${tenantId.value}/organization/companies/create`);
const goToCompany = (id: number) =>
  navigateTo(`/tenants/${tenantId.value}/organization/companies/${id}`);
const editOpen = ref(false);
const editingId = ref<number | null>(null);
const editForm = reactive<CompanyForm>({
  name: "",
  shortName: "",
  group: null,
  description: "",
...
const openEdit = (id: number) => {
  const company = storeCompanies.value.find((item) => item.id === id);
  if (!company) return;
  editingId.value = id;
  editForm.name = company.name;
  editForm.shortName = company.shortName;
  editForm.group = company.groupId ?? null;
  editForm.description = company.description;
  editForm.isActive = company.status === "active";
  editForm.createdBy = "—";
  editForm.createdAt = company.createdAt;
  editOpen.value = true;
};
const saveEdit = async (payload: CompanyForm) => {
  if (editingId.value === null) return;
  const company = storeCompanies.value.find(
  ...
  await companyStore.updateCompany(
    ...
    company.groupId ?? undefined,
    payload.group ?? undefined,
    company.business_entity_id,
  );
...
const confirmDelete = async () => {
  ...
  await companyStore.deleteCompany(deletingId.value);
...
<template>
    ...
    <div class="mx-auto w-full max-w-[1400px]">
      ...
      <div class="mb-5 flex items-center justify-between gap-4">
        <p class="text-sm text-gray-500 dark:text-gray-400">
          Organizasyon yapınızda yer alan şirketleri görüntüleyin ve yönetin.
        </p>
        <button
          type="button"
  ...
  /><ConfirmationModal
    ...
    title="Şirketi Sil"
    message="Bu şirketi silmek istediğinize emin misiniz? Bu işlem geri alınamaz."
    confirm-text="Sil"
    cancel-text="Vazgeç"
