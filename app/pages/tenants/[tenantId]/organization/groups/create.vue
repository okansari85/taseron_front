...
const rootOrganization = computed(() => organizationStore.holdings[0] ?? null)
...
onMounted(async () => {
  ...
  parentGroup.value = rootOrganization.value?.id ? String(rootOrganization.value.id) : ''
})

const saveGroup = async () => {
  if (!rootOrganization.value) {
    organizationStore.error = 'Bu tenant için grup oluşturulabilecek bir kök holding bulunamadı.'
    return
  }
  ...
  const payload: OrganizationPayload = {
    ...
    parent_id: rootOrganization.value.id,
