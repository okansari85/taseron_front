...
const paginatedGroups = computed(() => filteredGroups.value.slice(
  (currentPage.value - 1) * perPage.value,
  currentPage.value * perPage.value,
))
const visiblePages = computed(() => Array.from(
  { length: Math.min(5, totalPages.value) },
  (_, index) => Math.min(Math.max(1, currentPage.value - 2) + index, totalPages.value),
  ...
  <div class="font-outfit">
    <div class="mx-auto w-full max-w-[1400px]">
      ...
      <div class="mb-5 flex items-center justify-between gap-4">
        <p class="text-sm text-gray-500 dark:text-gray-400">Organizasyon yapınızda yer alan grupları görüntüleyin ve yönetin.</p>
      ...
      <section class="mb-5 rounded-xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="grid grid-cols-1 gap-3 md:grid-cols-[minmax(220px,1.2fr)_1fr_1fr_auto] md:items-center">
          <div class="relative">
            ...
            <input v-model="search" type="search" placeholder="Grup ara..." class="h-11 w-full rounded-lg border border-gray-200 bg-white pl-10 pr-3 text-sm text-gray-700 outline-none placeholder:text-gray-400 focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
          </div>
          <div class="relative">
          ...
          <div class="relative">
            <select v-model="parentFilter" class="h-11 w-full appearance-none rounded-lg border border-gray-200 bg-white px-3 pr-9 text-sm text-gray-700 outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
              <option value="all">Tümü</option><option value="root">Üst Seviye</option>
            </select>
            <ChevronDown :size="15" class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" />
          </div>
      ...
      <section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="overflow-x-auto">
          <table class="w-full min-w-[900px] text-left">
            <thead class="border-b border-gray-100 bg-gray-50/70 dark:border-gray-800 dark:bg-white/[0.03]">
              <tr><th class="px-4 py-4 text-xs font-medium text-gray-500 dark:text-gray-400">Grup Adı</th><th class="px-4 py-4 text-xs font-medium text-gray-500 dark:text-gray-400">Ebeveyn Grup</th><th class="px-4 py-4 text-xs font-medium text-gray-500 dark:text-gray-400">Açıklama</th><th class="px-4 py-4 text-xs font-medium text-gray-500 dark:text-gray-400">Durum</th><th class="px-4 py-4 text-xs font-medium text-gray-500 dark:text-gray-400">Oluşturulma Tarihi</th><th class="px-4 py-4 text-right text-xs font-medium text-gray-500 dark:text-gray-400">İşlemler</th></tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
              <tr v-for="group in paginatedGroups" :key="group.id" class="hover:bg-gray-50/70 dark:hover:bg-white/[0.02]">
                ...
                <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-400">{{ group.parent ?? '—' }}</td>
                <td class="max-w-[260px] px-4 py-4 text-sm leading-5 text-gray-500 dark:text-gray-400">{{ group.description }}</td>
                <td class="px-4 py-4"><span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium" :class="group.status === 'active' ? 'bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-300' : 'bg-gray-100 text-gray-500 dark:bg-gray-700/60 dark:text-gray-300'"><span class="h-1.5 w-1.5 rounded-full" :class="group.status === 'active' ? 'bg-success-500' : 'bg-gray-400'" />{{ group.status === 'active' ? 'Aktif' : 'Pasif' }}</span></td>
                <td class="whitespace-nowrap px-4 py-4 text-sm text-gray-500 dark:text-gray-400">{{ group.createdAt }}</td>
                <td class="px-4 py-4"><div class="flex items-center justify-end gap-1.5"><button type="button" class="flex h-9 w-9 items-center justify-center rounded-lg text-gray-400 transition hover:bg-brand-50 hover:text-brand-500 dark:hover:bg-brand-500/10" title="Görüntüle" aria-label="Görüntüle" @click="goToGroup(group.id)"><Eye :size="17" /></button><button type="button" class="flex h-9 w-9 items-center justify-center rounded-lg text-gray-400 transition hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-white/5 dark:hover:text-gray-200" title="Düzenle" aria-label="Düzenle" @click="openEdit(group)"><Pencil :size="16" /></button><button type="button" class="flex h-9 w-9 items-center justify-center rounded-lg text-gray-400 transition hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-white/5 dark:hover:text-gray-200" aria-label="Diğer işlemler"><EllipsisVertical :size="16" /></button></div></td>
              </tr>
              <tr v-if="paginatedGroups.length === 0"><td colspan="6" class="px-4 py-12 text-center"><div class="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-gray-400 dark:bg-white/[0.06]"><FolderTree :size="18" /></div><p class="mt-3 text-sm font-medium text-gray-700 dark:text-gray-300">Grup bulunamadı.</p></td></tr>
            </tbody>
          </table>
        </div>
        <div class="flex flex-col gap-3 border-t border-gray-100 px-4 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800">
          <span class="text-sm text-gray-500 dark:text-gray-400">Toplam {{ filteredGroups.length }} kayıt</span>
    ...
    <GroupEditDrawer
      ...
      :form="editForm"
      :parent-groups="organizationStore.groups.filter(item => item.id !== editingGroup?.id)"
