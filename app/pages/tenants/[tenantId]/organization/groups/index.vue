...
import { ChevronDown, ChevronLeft, ChevronRight, EllipsisVertical, Eye, Filter, FolderTree, Pencil, Plus, Search } from 'lucide-vue-next'
...
const hasHolding = computed(() => organizationStore.organizations.some(item => item.type === 'holding'))
...
const openEdit = (group: Group) => { editingGroup.value = group.raw; editForm.name = group.raw.name ?? ''; editForm.slug = group.raw.slug ?? ''; editForm.code = group.raw.code ?? ''; editForm.description = group.raw.description ?? ''; editForm.display_order = group.raw.display_order ?? 0; editForm.is_active = group.raw.is_active !== false && group.raw.is_active !== 0; editForm.parent_id = group.raw.parent_id ?? null; editForm.color = group.raw.color || '#465FFF'; editOpen.value = true }
...
<template>
  <div class="font-outfit">
    ...
    <GroupEditDrawer v-model:open="editOpen" :form="editForm" :parent-groups="organizationStore.groups.filter(item => item.id !== editingGroup?.id)" :has-holding="hasHolding" :saving="organizationStore.saving" @save="saveEdit" />
  </div>
</template>
