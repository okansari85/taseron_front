...
type BrandTone = 'red' | 'blue' | 'slate' | 'orange' | 'green' | 'yellow'
type Brand = { name: string; shortName: string; tone: BrandTone }
type Company = { name: string; shortName: string; brands: Brand[] }
type Group = { name: string; companies: Company[] }
...
const groups = ref<Group[]>([
  {
    ...
    companies: [
      ...
      { name: 'Arçelik Pazarlama A.Ş.', shortName: 'arcelik-pazarlama', brands: [] },
    ],
  },
  {
    name: 'Otomotiv Grubu',
    companies: [
      { name: 'Tofaş Türk Otomobil Fabrikası A.Ş.', shortName: 'tofas', brands: [{ name: 'Tofaş', shortName: 'tofas', tone: 'green' }] },
      { name: 'Otokar A.Ş.', shortName: 'otokar', brands: [{ name: 'Otokar', shortName: 'otokar', tone: 'yellow' }] },
    ],
  },
  {
    name: 'Enerji Grubu',
    companies: [
      { name: 'Tüpraş - Türkiye Petrol Rafinerileri A.Ş.', shortName: 'tupras', brands: [] },
    ],
  },
])
...
const toggleGroup = (name: string) => { expandedGroups.value[name] = !expandedGroups.value[name] }
const toggleCompany = (name: string) => { expandedCompanies.value[name] = !expandedCompanies.value[name] }
const companyInitial = (name: string) => name.trim().charAt(0).toLocaleUpperCase('tr-TR')
const brandToneClass = (tone: BrandTone) => ({
  red: 'bg-red-500',
  blue: 'bg-blue-500',
  slate: 'bg-slate-500',
    ...
    <div class="mb-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
      <div class="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex items-center gap-3">
          <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-50 text-brand-500 dark:bg-brand-500/10 dark:text-brand-400"><Layers3 :size="18" /></span>
      ...
      <div class="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex items-center gap-3">
          <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-success-50 text-success-600 dark:bg-success-500/10 dark:text-success-400"><Building2 :size="18" /></span>
      ...
      <div class="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex items-center gap-3">
          <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-light-50 text-blue-light-600 dark:bg-blue-light-500/10 dark:text-blue-light-400"><Network :size="18" /></span>
    ...
    <section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
      <div class="border-b border-gray-100 px-6 py-5 dark:border-gray-800">
        <div class="flex items-center justify-between gap-4">
          ...
          <span class="hidden items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-medium text-gray-500 sm:inline-flex dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400"><Network :size="14" /> Salt okunur</span>
        </div>
      </div>
        ...
        <div class="min-w-[900px]">
          <div class="mx-auto flex w-fit flex-col items-center">
            <div class="flex min-w-[300px] items-center gap-3 rounded-xl border border-brand-200 bg-brand-50/70 px-5 py-4 shadow-theme-xs dark:border-brand-500/30 dark:bg-brand-500/10">
              <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-brand-500 text-white"><Network :size="20" /></span>
              <div><div class="text-sm font-semibold text-gray-900 dark:text-white/90">Koç Holding</div><div class="mt-0.5 text-xs text-brand-600 dark:text-brand-400">Holding</div></div>
            </div>
            ...
            <div class="relative flex w-full justify-center gap-8 before:absolute before:left-[16.66%] before:right-[16.66%] before:top-0 before:h-px before:bg-gray-200 dark:before:bg-gray-700">
              <div v-for="group in groups" :key="group.name" class="relative w-[280px] pt-6">
                <div class="absolute left-1/2 top-0 h-6 w-px -translate-x-1/2 bg-gray-200 dark:bg-gray-700" />

                <button type="button" class="group flex w-full items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3.5 text-left shadow-theme-xs transition hover:border-brand-200 hover:shadow-md dark:border-gray-700 dark:bg-gray-900 dark:hover:border-brand-500/30" @click="toggleGroup(group.name)">
                  ...
                  <span class="min-w-0 flex-1"><span class="block truncate text-sm font-semibold text-gray-800 dark:text-white/90">{{ group.name }}</span><span class="mt-0.5 block text-xs text-gray-500 dark:text-gray-400">{{ group.companies.length }} şirket</span></span>
                  <ChevronDown v-if="expandedGroups[group.name]" :size="16" class="shrink-0 text-gray-400" /><ChevronRight v-else :size="16" class="shrink-0 text-gray-400" />
                </button>
                ...
                <div v-if="expandedGroups[group.name]" class="relative mt-4 space-y-3 pl-5 before:absolute before:bottom-6 before:left-1 before:top-0 before:w-px before:bg-gray-200 dark:before:bg-gray-700">
                  <div v-for="company in group.companies" :key="company.name" class="relative">
                    <div class="absolute -left-4 top-5 h-px w-4 bg-gray-200 dark:bg-gray-700" />
                    <button type="button" class="flex w-full items-center gap-3 rounded-lg border border-gray-200 bg-white px-3.5 py-3 text-left transition hover:border-success-200 hover:bg-success-50/30 dark:border-gray-700 dark:bg-gray-900 dark:hover:border-success-500/30 dark:hover:bg-success-500/5" @click="toggleCompany(company.name)">
                      <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-success-50 text-xs font-semibold text-success-600 dark:bg-success-500/10 dark:text-success-400">{{ companyInitial(company.name) }}</span>
                      <span class="min-w-0 flex-1"><span class="block truncate text-sm font-medium text-gray-800 dark:text-white/90">{{ company.name }}</span><span class="mt-0.5 block text-xs text-gray-500 dark:text-gray-400">{{ company.shortName }}</span></span>
                      <ChevronDown v-if="expandedCompanies[company.name]" :size="15" class="shrink-0 text-gray-400" /><ChevronRight v-else :size="15" class="shrink-0 text-gray-400" />
                    </button>
                    ...
                    <div v-if="expandedCompanies[company.name] && company.brands.length" class="mt-2 ml-5 space-y-1.5 border-l border-gray-200 pl-4 dark:border-gray-700">
                      <div v-for="brand in company.brands" :key="brand.name" class="flex items-center gap-2.5 rounded-lg px-3 py-2 transition hover:bg-gray-50 dark:hover:bg-white/[0.03]">
                        <span class="h-2 w-2 rounded-full" :class="brandToneClass(brand.tone)" />
