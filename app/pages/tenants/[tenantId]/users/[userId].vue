      ...
      <div class="grid grid-cols-1 gap-5 lg:grid-cols-[300px_1fr]">
        <div class="space-y-5">
          ...
          <div class="rounded-xl border border-gray-200 bg-white p-2">
            <button @click="activeTab = 'profile'" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors" :class="activeTab === 'profile' ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50'">
              <UserRound :size="16" :class="activeTab === 'profile' ? 'text-brand-600' : 'text-gray-400'" />
              <div><p class="text-xs font-semibold">Profil</p><p class="mt-0.5 text-[10px] text-gray-400">Kullanıcı bilgileri</p></div>
            </button>
            <button @click="activeTab = 'roles'" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors" :class="activeTab === 'roles' ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50'">
              <ShieldCheck :size="16" :class="activeTab === 'roles' ? 'text-brand-600' : 'text-gray-400'" />
              <div><p class="text-xs font-semibold">Roller</p><p class="mt-0.5 text-[10px] text-gray-400">Kullanıcı rolü</p></div>
            </button>
            <button @click="activeTab = 'scopes'" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors" :class="activeTab === 'scopes' ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50'">
              <ShieldCheck :size="16" :class="activeTab === 'scopes' ? 'text-brand-600' : 'text-gray-400'" />
              <div><p class="text-xs font-semibold">Kapsamlar</p><p class="mt-0.5 text-[10px] text-gray-400">Erişim alanları</p></div>
            </button>
          </div>
        ...
        <div class="space-y-5">
          <div v-if="activeTab === 'profile'" class="rounded-xl border border-gray-200 bg-white p-5">
            <div class="flex items-center gap-2"><UserRound :size="16" class="text-brand-600" /><h2 class="text-sm font-semibold text-gray-900">Kullanıcı Bilgileri</h2></div>
            <div class="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
              <div class="rounded-lg border border-gray-100 p-4"><p class="text-[11px] text-gray-400">Ad Soyad</p><p class="mt-1 text-sm font-medium text-gray-800">{{ user.name }}</p></div>
              <div class="rounded-lg border border-gray-100 p-4"><p class="text-[11px] text-gray-400">E-posta</p><p class="mt-1 text-sm font-medium text-gray-800">{{ user.email }}</p></div>
              <div class="rounded-lg border border-gray-100 p-4"><p class="text-[11px] text-gray-400">Rol</p><p class="mt-1 text-sm font-medium text-gray-800">{{ user.roles?.[0]?.name ?? 'Rol atanmadı' }}</p></div>
              <div class="rounded-lg border border-gray-100 p-4"><p class="text-[11px] text-gray-400">Durum</p><p class="mt-1 text-sm font-medium text-gray-800">{{ user.status ?? '—' }}</p></div>
            </div>
          </div>
          ...
          <div v-else-if="activeTab === 'roles'">
            ...
            <div v-else class="rounded-xl border border-gray-200 bg-white p-5">
              <div><h2 class="text-sm font-semibold text-gray-900">Rol</h2><p class="mt-1 text-xs text-gray-500">Kullanıcının platform içindeki yetki seviyesini belirleyin.</p></div>
              <select v-model="selectedRole" class="mt-5 h-10 w-full rounded-lg border border-gray-200 bg-white px-3 text-xs outline-none"><option value="">Rol seçin</option><option v-for="role in roles" :key="role.id" :value="role.name">{{ role.name }}</option></select>
              <div class="mt-5 flex justify-end gap-2"><button @click="router.push(`/tenants/${tenantId}/users`)" class="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-xs font-medium text-gray-600 hover:bg-gray-50"><X :size="14" /> Vazgeç</button><button :disabled="savingRole" @click="saveRole" class="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2 text-xs font-medium text-white disabled:opacity-60"><LoaderCircle v-if="savingRole" :size="14" class="animate-spin" /><Check v-else :size="14" />{{ savingRole ? 'Kaydediliyor...' : 'Rolü Kaydet' }}</button></div>
            </div>
          </div>
