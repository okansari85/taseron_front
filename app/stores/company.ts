...
export const useCompanyStore = defineStore('company', () => {
  ...
  const updateCompany = async (id: number, payload: { name: string; company_type: 'corporate'; short_name: string; description: string; is_active: boolean }, organizationId?: number, businessEntityId?: number | null) => { saving.value = true; error.value = null; try { const response = await companyApi.update(id, payload); if (organizationId && businessEntityId) await organizationCompanyApi.sync(organizationId, [businessEntityId]); const index = companies.value.findIndex(item => item.id === id); if (index !== -1) companies.value[index] = normalizeCompany({ ...response, organizations: organizationId ? [{ id: organizationId, name: companies.value[index].group, type: 'group' }] : undefined }); return response } catch (err) { error.value = err instanceof Error ? err.message : 'Şirket güncellenemedi.'; throw err } finally { saving.value = false } }
